...
| `DEP-R` | Static records that are neither active research nor cold-storage artifacts. | `DEP-R/DEP-R-YYYYMMDD-Short Description/` |

Do not file a class-prefixed DEP entry directly under `.lake-data`. Place each entry inside its matching `DEP-E`, `DEP-R`, or `DEP-A` class container.
...
2. Create the entry inside the matching class container using the required class prefix and date: `DEP-E/DEP-E-YYYYMMDD-Short Description/`, `DEP-R/DEP-R-YYYYMMDD-Short Description/`, or `DEP-A/DEP-A-YYYYMMDD-Short Description/`.
...
7. Verify that the entry path exists, each publication row resolves to an owning DEP, and no root-level class-prefixed DEP entry remains.
