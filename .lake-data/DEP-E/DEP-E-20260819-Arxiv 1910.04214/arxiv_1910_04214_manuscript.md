---
title: "arXiv 1910.04214 - DEP-E"
generated_at: "2026-08-19"
artifact_type: "DEP research artifact"
primary_subject: "Source-grounded review of [1910.04214] Who’s responsible? Jointly quantifying the contribution of the learning algorithm and data."
...
primary_url: "https://arxiv.org/abs/1910.04214"
stable_identifier: "arXiv:1910.04214"
...
# arXiv 1910.04214 - DEP-E

## Source Metadata

| Field | Value |
|---|---|
| Paper title | [1910.04214] Who’s responsible? Jointly quantifying the contribution of the learning algorithm and data |
| Authors | Not available from inspected sources |
| Platform | arXiv |
| arXiv ID | 1910.04214 |
| Revision/publication date | Not available from inspected sources |
| Primary URLs | https://arxiv.org/abs/1910.04214; https://arxiv.org/html/1910.04214; https://arxiv.org/pdf/1910.04214 |
...
|---|---|---|---|---|---|---|
| E1 | https://arxiv.org/html/1910.04214 | Primary paper HTML | Full-paper title, abstract, headings, method/evidence text, and limitations cues | Research object, method, evidence, and reviewer notes | High | Text extraction may omit visual layout and fine-grained table context. |
| E2 | https://arxiv.org/pdf/1910.04214 | Primary paper PDF | PDF integrity gate and source-presence cross-check | Complete-source status | High | No independent reproduction or figure-by-figure audit in this batch pass. |
| E3 | https://arxiv.org/abs/1910.04214 | Canonical metadata record | arXiv identifier and public locator | Stable identity and provenance | High | Abstract page is metadata only and was not treated as the full paper. |

## Executive Summary

The reviewed paper, *[1910.04214] Who’s responsible? Jointly quantifying the contribution of the learning algorithm and data*, addresses a research problem described in its full-paper HTML as follows: Abstract A learning algorithm A 𝐴 A trained on a dataset D 𝐷 D is revealed to have poor performance on some subpopulation at test time. Where should the responsibility for this lay? It can be argued that the data is responsible, if for example training A 𝐴 A on a more representative dataset D ′ superscript 𝐷 ′ D^{\prime} would have improved the performance. But it can similarly be argued that A 𝐴 A itself is at fault, if training a different variant A ′ superscript 𝐴 ′ A^{\prime} on the same dataset D 𝐷 D would have improved performance. As ML becomes widespread and such failure cases more common, these types of questions are proving to be far from hypothetical. With this motivation in mind, in this work we provide a rigorous formulation of the joint credit assignment problem between a learning algorithm A 𝐴 A and a dataset D 𝐷 D . We propose Extended Shapley as a principled framework for this problem, and experiment empirically with how it can be used to address questions of ML accountability. The source presents the paper's contribution through the following visible section structure: Who’s responsible? Jointly quantifying the contribution of the learning algorithm and data, Abstract, 1 Introduction, 1.1 Our contributions, 1.1.1 Related work, 2 Jointly modeling the contribution of data and algorithm, 2.1 Data valuation, 2.2 Data and learning algorithm valuation, Warm-up: Symmetric version., 3 Extended Shapley, Proposition 3.1 ., Interpretation of the Extended Shapley values.. Reviewer interpretation: the work is most useful as a source-grounded design or evaluation reference, while the exact strength of its claims depends on the experiments, datasets, baselines, and boundary conditions reported in the paper. Full-paper source integrity passed; independent reproduction was not performed.

## Detailed Summary

### Problem and contribution

The paper frames its problem and motivation in the primary source. The extracted abstract is: Abstract A learning algorithm A 𝐴 A trained on a dataset D 𝐷 D is revealed to have poor performance on some subpopulation at test time. Where should the responsibility for this lay? It can be argued that the data is responsible, if for example training A 𝐴 A on a more representative dataset D ′ superscript 𝐷 ′ D^{\prime} would have improved the performance. But it can similarly be argued that A 𝐴 A itself is at fault, if training a different variant A ′ superscript 𝐴 ′ A^{\prime} on the same dataset D 𝐷 D would have improved performance. As ML becomes widespread and such failure cases more common, these types of questions are proving to be far from hypothetical. With this motivation in mind, in this work we provide a rigorous formulation of the joint credit assignment problem between a learning algorithm A 𝐴 A and a dataset D 𝐷 D . We propose Extended Shapley as a principled framework for this problem, and experiment empirically with how it can be used to address questions of ML accountability.

### Method and mechanism

The full-paper text contains the following method-related evidence: Jointly quantifying the contribution of the learning algorithm and data Who’s responsible? Jointly quantifying the contribution of the learning algorithm and data Gal Yona Weizmann Institute of Science gal.yona@weizmann.ac.il Amirata Ghorbani Stanford University gal.yona@weizmann.ac.il James Zou Stanford University gal.yona@weizmann.ac.il Gal Yona Weizmann Institute of Science Amirata Ghorbani James Zou Stanford... The available heading structure is retained as a navigation aid rather than treated as proof of every technical claim.

### Evidence and results

Evidence-related text includes: Jointly quantifying the contribution of the learning algorithm and data Gal Yona Weizmann Institute of Science gal.yona@weizmann.ac.il Amirata Ghorbani Stanford University gal.yona@weizmann.ac.il James Zou Stanford University gal.yona@weizmann.ac.il Gal Yona Weizmann Institute of Science Amirata Ghorbani James Zou Stanford University Correspondence: jamesz@stanford.edu Abstract A learning algorithm A 𝐴 A trained... Exact metrics and comparisons should be checked against the paper's tables and figures before making deployment decisions.

### Limitations and conclusion

Limitation-related text includes: As ML becomes widespread and such failure cases more common, these types of questions are proving to be far from hypothetical. These findings sparked internet controversy around the source of these failures (see [ Gradient, 2020 ] for a comprehensive overview). Where the source did not expose a clear limitation in the extracted text, this review records the gap rather than inferring that no limitation exists.
...
- `Sources inspected`: The local verified full-paper PDF and full-paper HTML for arXiv:1910.04214; canonical arXiv metadata URL; three existing Black Lake DEP entries for related context.
...
- `Batch selection`: 75967 PDF candidates; 894 integrity-valid source units; 865 valid identified IDs; 2084 IDs found in repository/memory dedup scans; 396 duplicate exclusions; 0 reselections; 469 unseen eligible IDs; this item is sample position 135 of 200.
...
- `Temporal boundary`: Public deposition date 2026-08-19; source version is the locally archived arXiv version identified as 1910.04214.
...
|---|---|---|---|
| DEP-E-20260721-Dataset Baselines | .lake-data/DEP-E/DEP-E-20260721-Dataset Baselines | Shared terms: data, dataset | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-E/DEP-E-20260721-Dataset%20Baselines/README.md |
...
|---|---|---|---|---|
| S1 | https://arxiv.org/html/1910.04214 | Full-paper text and section-level review | 2026-08-19 | Primary public locator; source file withheld locally. |
| S2 | https://arxiv.org/pdf/1910.04214 | PDF/source integrity status | 2026-08-19 | Primary public locator; source file withheld locally. |
| S3 | https://arxiv.org/abs/1910.04214 | Canonical metadata and stable identifier | 2026-08-19 | Metadata page only; not treated as the paper document. |
| S4 | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-E/DEP-E-20260721-Dataset%20Baselines/README.md | Related DEP context: data, dataset | 2026-08-19 | Repository file used for conceptual context. |
...
- https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-E/DEP-E-20260721-Dataset%20Baselines/README.md — repository context entry `.lake-data/DEP-E/DEP-E-20260721-Dataset Baselines`; selected for conceptual overlap indicated by data, dataset; not used as primary evidence.
