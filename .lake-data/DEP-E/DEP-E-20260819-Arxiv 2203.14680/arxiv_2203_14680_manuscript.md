---
title: "arXiv 2203.14680 - DEP-E"
generated_at: "2026-08-19"
artifact_type: "DEP research artifact"
primary_subject: "Source-grounded review of [2203.14680] Transformer Feed-Forward Layers Build Predictions by Promoting Concepts in the Vocabulary Space."
...
primary_url: "https://arxiv.org/abs/2203.14680"
stable_identifier: "arXiv:2203.14680"
...
# arXiv 2203.14680 - DEP-E

## Source Metadata

| Field | Value |
|---|---|
| Paper title | [2203.14680] Transformer Feed-Forward Layers Build Predictions by Promoting Concepts in the Vocabulary Space |
| Authors | Not available from inspected sources |
| Platform | arXiv |
| arXiv ID | 2203.14680 |
| Revision/publication date | Not available from inspected sources |
| Primary URLs | https://arxiv.org/abs/2203.14680; https://arxiv.org/html/2203.14680; https://arxiv.org/pdf/2203.14680 |
...
|---|---|---|---|---|---|---|
| E1 | https://arxiv.org/html/2203.14680 | Primary paper HTML | Full-paper title, abstract, headings, method/evidence text, and limitations cues | Research object, method, evidence, and reviewer notes | High | Text extraction may omit visual layout and fine-grained table context. |
| E2 | https://arxiv.org/pdf/2203.14680 | Primary paper PDF | PDF integrity gate and source-presence cross-check | Complete-source status | High | No independent reproduction or figure-by-figure audit in this batch pass. |
| E3 | https://arxiv.org/abs/2203.14680 | Canonical metadata record | arXiv identifier and public locator | Stable identity and provenance | High | Abstract page is metadata only and was not treated as the full paper. |

## Executive Summary

The reviewed paper, *[2203.14680] Transformer Feed-Forward Layers Build Predictions by Promoting Concepts in the Vocabulary Space*, addresses a research problem described in its full-paper HTML as follows: Abstract Transformer-based language models (LMs) are at the core of modern NLP, but their internal prediction construction process is opaque and largely not understood. In this work, we make a substantial step towards unveiling this underlying prediction process, by reverse-engineering the operation of the feed-forward network (FFN) layers, one of the building blocks of transformer models. We view the token representation as a changing distribution over the vocabulary, and the output from each FFN layer as an additive update to that distribution. Then, we analyze the FFN updates in the vocabulary space, showing that each update can be decomposed to sub-updates corresponding to single FFN parameter vectors, each promoting concepts that are often human-interpretable. We then leverage these findings for controlling LM predictions, where we reduce the toxicity of GPT2 by almost 50%, and for improving computation efficiency with a simple early exit rule, saving 20% of computation on average. 1 1 1 Our codebase is available at https://github.com/aviclu/ffn-values . The source presents the paper's contribution through the following visible section structure: Transformer Feed-Forward Layers Build Predictions by Promoting Concepts in the Vocabulary Space, Abstract, 1 Introduction, 2 Token Representations as Evolving Distributions Over the Vocabulary, 3 The FFN Output as a Collection of Updates to the Output Distribution, FFN Outputs as Linear Vector Combinations., Terminology., Interpreting Sub-Updates in the Vocabulary Space., 4 Sub-Updates Encode Concepts in the Vocabulary Space, Concepts Annotation Task., Models., 4.1 Projection of Sub-Updates is Meaningful. Reviewer interpretation: the work is most useful as a source-grounded design or evaluation reference, while the exact strength of its claims depends on the experiments, datasets, baselines, and boundary conditions reported in the paper. Full-paper source integrity passed; independent reproduction was not performed.

## Detailed Summary

### Problem and contribution

The paper frames its problem and motivation in the primary source. The extracted abstract is: Abstract Transformer-based language models (LMs) are at the core of modern NLP, but their internal prediction construction process is opaque and largely not understood. In this work, we make a substantial step towards unveiling this underlying prediction process, by reverse-engineering the operation of the feed-forward network (FFN) layers, one of the building blocks of transformer models. We view the token representation as a changing distribution over the vocabulary, and the output from each FFN layer as an additive update to that distribution. Then, we analyze the FFN updates in the vocabulary space, showing that each update can be decomposed to sub-updates corresponding to single FFN parameter vectors, each promoting concepts that are often human-interpretable. We then leverage these findings for controlling LM predictions, where we reduce the toxicity of GPT2 by almost 50%, and for improving computation efficiency with a simple early exit rule, saving 20% of computation on average. 1 1 1 Our codebase is available at https://github.com/aviclu/ffn-values .

### Method and mechanism

The full-paper text contains the following method-related evidence: which both WikiLM and GPT2 did not observe during training. We utilize our findings to create a simple, intuitive method for toxic language suppression. The available heading structure is retained as a navigation aid rather than treated as proof of every technical claim.

### Evidence and results

Evidence-related text includes: Through a series of experiments, we find that (a) sub-update vectors across the entire network often encode a small-set of human-interpretable well-defined concepts, e.g. A transformer LM with L = 10 , d m = 3000 formulae-sequence 𝐿 10 subscript 𝑑 𝑚 3000 L=10,d_{m}=3000 will have 30 , 000 30 000 30,000 value vectors, and every token that passes through the transformer will weight these value vectors differently,... Exact metrics and comparisons should be checked against the paper's tables and figures before making deployment decisions.

### Limitations and conclusion

Limitation-related text includes: Model Toxicity Severe Sexually Threat Profanity Identity PPL toxicity explicit attack GPT2 58.5% 49.2% 34.1% 16.4% 52.5% 16.8% 21.7 ↑ ↑ \uparrow 10 Manual Pick 30.8% 24.8% 20.4% 6.0% 27.9% 8.8% 25.3 ↑ ↑ \uparrow 10 API Graded 52.7% 44% 33.2% 13.3% 47.6% 15.3% 23.8 SD 37.2% 26.4% 21.7% 7.8% 32% 8.4% 23.9 WordFilter 46.9% 32.4% 21.9% 16.3% 32.3% 14.7% - Table 5: Evaluation results on the challenging subset of... Where the source did not expose a clear limitation in the extracted text, this review records the gap rather than inferring that no limitation exists.
...
- `Sources inspected`: The local verified full-paper PDF and full-paper HTML for arXiv:2203.14680; canonical arXiv metadata URL; three existing Black Lake DEP entries for related context.
...
- `Batch selection`: 75967 PDF candidates; 894 integrity-valid source units; 865 valid identified IDs; 2084 IDs found in repository/memory dedup scans; 396 duplicate exclusions; 0 reselections; 469 unseen eligible IDs; this item is sample position 170 of 200.
...
- `Temporal boundary`: Public deposition date 2026-08-19; source version is the locally archived arXiv version identified as 2203.14680.
...
|---|---|---|---|
| DEP-A-20260728-Language to Space Intake | .lake-data/DEP-A/DEP-A-20260728-Language to Space Intake | Shared terms: space, language | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260728-Language%20to%20Space%20Intake/README.md |
...
| DEP-E-20260727-Language-to-Space | .lake-data/DEP-E/DEP-E-20260727-Language-to-Space | Shared terms: space, language | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-E/DEP-E-20260727-Language-to-Space/README.md |
...
|---|---|---|---|---|
| S1 | https://arxiv.org/html/2203.14680 | Full-paper text and section-level review | 2026-08-19 | Primary public locator; source file withheld locally. |
| S2 | https://arxiv.org/pdf/2203.14680 | PDF/source integrity status | 2026-08-19 | Primary public locator; source file withheld locally. |
| S3 | https://arxiv.org/abs/2203.14680 | Canonical metadata and stable identifier | 2026-08-19 | Metadata page only; not treated as the paper document. |
| S4 | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260728-Language%20to%20Space%20Intake/README.md | Related DEP context: space, language | 2026-08-19 | Repository file used for conceptual context. |
...
| S6 | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-E/DEP-E-20260727-Language-to-Space/README.md | Related DEP context: space, language | 2026-08-19 | Repository file used for conceptual context. |
...
- https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260728-Language%20to%20Space%20Intake/README.md — repository context entry `.lake-data/DEP-A/DEP-A-20260728-Language to Space Intake`; selected for conceptual overlap indicated by space, language; not used as primary evidence.
...
- https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-E/DEP-E-20260727-Language-to-Space/README.md — repository context entry `.lake-data/DEP-E/DEP-E-20260727-Language-to-Space`; selected for conceptual overlap indicated by space, language; not used as primary evidence.
