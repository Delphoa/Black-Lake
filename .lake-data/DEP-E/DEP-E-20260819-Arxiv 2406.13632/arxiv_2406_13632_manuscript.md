---
title: "arXiv 2406.13632 - DEP-E"
generated_at: "2026-08-19"
artifact_type: "DEP research artifact"
primary_subject: "Source-grounded review of DoubleDipper: Improving Long-Context LLMs via Context Recycling."
...
primary_url: "https://arxiv.org/abs/2406.13632"
stable_identifier: "arXiv:2406.13632"
...
# arXiv 2406.13632 - DEP-E

## Source Metadata

| Field | Value |
|---|---|
| Paper title | DoubleDipper: Improving Long-Context LLMs via Context Recycling |
| Authors | Not available from inspected sources |
| Platform | arXiv |
| arXiv ID | 2406.13632 |
| Revision/publication date | Not available from inspected sources |
| Primary URLs | https://arxiv.org/abs/2406.13632; https://arxiv.org/html/2406.13632; https://arxiv.org/pdf/2406.13632 |
...
|---|---|---|---|---|---|---|
| E1 | https://arxiv.org/html/2406.13632 | Primary paper HTML | Full-paper title, abstract, headings, method/evidence text, and limitations cues | Research object, method, evidence, and reviewer notes | High | Text extraction may omit visual layout and fine-grained table context. |
| E2 | https://arxiv.org/pdf/2406.13632 | Primary paper PDF | PDF integrity gate and source-presence cross-check | Complete-source status | High | No independent reproduction or figure-by-figure audit in this batch pass. |
| E3 | https://arxiv.org/abs/2406.13632 | Canonical metadata record | arXiv identifier and public locator | Stable identity and provenance | High | Abstract page is metadata only and was not treated as the full paper. |

## Executive Summary

The reviewed paper, *DoubleDipper: Improving Long-Context LLMs via Context Recycling*, addresses a research problem described in its full-paper HTML as follows: Abstract Despite recent advancements in Large Language Models (LLMs), their performance on tasks involving long contexts remains sub-optimal. In this work, we propose DoubleDipper , a novel In-Context-Learning method that automatically generates few-shot examples for long context QA tasks by recycling contexts. Specifically, given a long input context (1-3k tokens) and a query, we generate additional query-output pairs from the given context as few-shot examples, while introducing the context only once. This ensures that the demonstrations are leveraging the same context as the target query while only adding a small number of tokens to the prompt. We further enhance each demonstration by instructing the model to explicitly identify the relevant paragraphs before the answer, which improves performance while providing fine-grained attribution to the answer source. We apply our method on multiple LLMs and obtain substantial improvements (+16 absolute points on average across models) on various QA datasets with long context. Surprisingly, despite introducing only single-hop ICL examples, LLMs successfully generalize to multi-hop long-context QA using our approach. The source presents the paper's contribution through the following visible section structure: DoubleDipper : Improving Long-Context LLMs via Context Recycling, Abstract, 1 Introduction, 2 Background, Challenges in Long Context for Language Modeling., In-Context Learning, 3 DoubleDipper, 4 Experiments, Datasets, Models, Baselines, Evaluation. Reviewer interpretation: the work is most useful as a source-grounded design or evaluation reference, while the exact strength of its claims depends on the experiments, datasets, baselines, and boundary conditions reported in the paper. Full-paper source integrity passed; independent reproduction was not performed.
...
The full-paper text contains the following method-related evidence: In this work, we propose DoubleDipper , a novel In-Context-Learning method that automatically generates few-shot examples for long context QA tasks by recycling contexts. We apply our method on multiple LLMs and obtain substantial improvements (+16 absolute points on average across models) on various QA datasets with long context. The available heading structure is retained as a navigation aid rather than treated as proof of every technical claim.

### Evidence and results

Evidence-related text includes: In-Context Learning 3 DoubleDipper 4 Experiments Datasets Models Baselines Evaluation Implementation Details 5 Results Result 1: DoubleDipper offers a substantial performance boost. Result 2: Learning to retrieve the evidence(s) with DoubleDipper is more effective in commercial and large open source models. Exact metrics and comparisons should be checked against the paper's tables and figures before making deployment decisions.

### Limitations and conclusion

Limitation-related text includes: Traditional ICL Qualitative analysis: Correctness of the generated QA pairs 7 Conclusion 8 Limitations A Appendix B Prompts C Additional Results D Lost-in-the-middle E Analysis E.1 Impact of the Number of Demonstrations in DoubleDipper E.2 Impact of the few-shot generator Can we generate demonstrations with smaller LLMs? 8 Limitations Our work has several limitations. Where the source did not expose a clear limitation in the extracted text, this review records the gap rather than inferring that no limitation exists.
...
- `Sources inspected`: The local verified full-paper PDF and full-paper HTML for arXiv:2406.13632; canonical arXiv metadata URL; three existing Black Lake DEP entries for related context.
...
- `Batch selection`: 75967 PDF candidates; 894 integrity-valid source units; 865 valid identified IDs; 2084 IDs found in repository/memory dedup scans; 396 duplicate exclusions; 0 reselections; 469 unseen eligible IDs; this item is sample position 116 of 200.
...
- `Temporal boundary`: Public deposition date 2026-08-19; source version is the locally archived arXiv version identified as 2406.13632.
...
|---|---|---|---|
| DEP-A-20260716-Performance Context | .lake-data/DEP-A/DEP-A-20260716-Performance Context | Shared terms: context, performance | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260716-Performance%20Context/README.md |
| DEP-A-20260720-ContextSniper Code | .lake-data/DEP-A/DEP-A-20260720-ContextSniper Code | Shared terms: context, contexts | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260720-ContextSniper%20Code/README.md |
| DEP-A-20260726-MemReread Long Context | .lake-data/DEP-A/DEP-A-20260726-MemReread Long Context | Shared terms: long, context | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260726-MemReread%20Long%20Context/README.md |
...
|---|---|---|---|---|
| S1 | https://arxiv.org/html/2406.13632 | Full-paper text and section-level review | 2026-08-19 | Primary public locator; source file withheld locally. |
| S2 | https://arxiv.org/pdf/2406.13632 | PDF/source integrity status | 2026-08-19 | Primary public locator; source file withheld locally. |
| S3 | https://arxiv.org/abs/2406.13632 | Canonical metadata and stable identifier | 2026-08-19 | Metadata page only; not treated as the paper document. |
| S4 | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260716-Performance%20Context/README.md | Related DEP context: context, performance | 2026-08-19 | Repository file used for conceptual context. |
| S5 | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260720-ContextSniper%20Code/README.md | Related DEP context: context, contexts | 2026-08-19 | Repository file used for conceptual context. |
| S6 | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260726-MemReread%20Long%20Context/README.md | Related DEP context: long, context | 2026-08-19 | Repository file used for conceptual context. |
...
- https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260716-Performance%20Context/README.md — repository context entry `.lake-data/DEP-A/DEP-A-20260716-Performance Context`; selected for conceptual overlap indicated by context, performance; not used as primary evidence.
- https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260720-ContextSniper%20Code/README.md — repository context entry `.lake-data/DEP-A/DEP-A-20260720-ContextSniper Code`; selected for conceptual overlap indicated by context, contexts; not used as primary evidence.
- https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260726-MemReread%20Long%20Context/README.md — repository context entry `.lake-data/DEP-A/DEP-A-20260726-MemReread Long Context`; selected for conceptual overlap indicated by long, context; not used as primary evidence.
