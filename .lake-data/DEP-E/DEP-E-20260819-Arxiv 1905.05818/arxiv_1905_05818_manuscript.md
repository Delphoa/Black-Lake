---
title: "arXiv 1905.05818 - DEP-E"
generated_at: "2026-08-19"
artifact_type: "DEP research artifact"
primary_subject: "Source-grounded review of [1905.05818] Ontology-Aware Clinical Abstractive Summarization."
...
primary_url: "https://arxiv.org/abs/1905.05818"
stable_identifier: "arXiv:1905.05818"
...
# arXiv 1905.05818 - DEP-E

## Source Metadata

| Field | Value |
|---|---|
| Paper title | [1905.05818] Ontology-Aware Clinical Abstractive Summarization |
| Authors | Not available from inspected sources |
| Platform | arXiv |
| arXiv ID | 1905.05818 |
| Revision/publication date | Not available from inspected sources |
| Primary URLs | https://arxiv.org/abs/1905.05818; https://arxiv.org/html/1905.05818; https://arxiv.org/pdf/1905.05818 |
...
|---|---|---|---|---|---|---|
| E1 | https://arxiv.org/html/1905.05818 | Primary paper HTML | Full-paper title, abstract, headings, method/evidence text, and limitations cues | Research object, method, evidence, and reviewer notes | High | Text extraction may omit visual layout and fine-grained table context. |
| E2 | https://arxiv.org/pdf/1905.05818 | Primary paper PDF | PDF integrity gate and source-presence cross-check | Complete-source status | High | No independent reproduction or figure-by-figure audit in this batch pass. |
| E3 | https://arxiv.org/abs/1905.05818 | Canonical metadata record | arXiv identifier and public locator | Stable identity and provenance | High | Abstract page is metadata only and was not treated as the full paper. |

## Executive Summary

The reviewed paper, *[1905.05818] Ontology-Aware Clinical Abstractive Summarization*, addresses a research problem described in its full-paper HTML as follows: Abstract. Automatically generating accurate summaries from clinical reports could save a clinician’s time, improve summary coverage, and reduce errors. We propose a sequence-to-sequence abstractive summarization model augmented with domain-specific ontological information to enhance content selection and summary generation. We apply our method to a dataset of radiology reports and show that it significantly outperforms the current state-of-the-art on this task in terms of rouge scores. Extensive human evaluation conducted by a radiologist further indicates that this approach yields summaries that are less likely to omit important details, without sacrificing readability or accuracy. The source presents the paper's contribution through the following visible section structure: Ontology-Aware Clinical Abstractive Summarization, Abstract., 1. Introduction, 1.1. Related Work, 2. Model, 3. Experimental setup, 4. Results and Analysis, 4.1. Experimental results, 4.2. Expert human evaluation, 5. Conclusion, Acknowledgments, References. Reviewer interpretation: the work is most useful as a source-grounded design or evaluation reference, while the exact strength of its claims depends on the experiments, datasets, baselines, and boundary conditions reported in the paper. Full-paper source integrity passed; independent reproduction was not performed.

## Detailed Summary

### Problem and contribution

The paper frames its problem and motivation in the primary source. The extracted abstract is: Abstract. Automatically generating accurate summaries from clinical reports could save a clinician’s time, improve summary coverage, and reduce errors. We propose a sequence-to-sequence abstractive summarization model augmented with domain-specific ontological information to enhance content selection and summary generation. We apply our method to a dataset of radiology reports and show that it significantly outperforms the current state-of-the-art on this task in terms of rouge scores. Extensive human evaluation conducted by a radiologist further indicates that this approach yields summaries that are less likely to omit important details, without sacrificing readability or accuracy.

### Method and mechanism

The full-paper text contains the following method-related evidence: We apply our method to a dataset of radiology reports and show that it significantly outperforms the current state-of-the-art on this task in terms of rouge scores. Further extensive human evaluation by a radiology expert demonstrates that our method produces more complete summaries than the top-performing baseline, while not sacrificing readability or accuracy. The available heading structure is retained as a navigation aid rather than treated as proof of every technical claim.

### Evidence and results

Evidence-related text includes: Automatically generating accurate summaries from clinical reports could save a clinician’s time, improve summary coverage, and reduce errors. We apply our method to a dataset of radiology reports and show that it significantly outperforms the current state-of-the-art on this task in terms of rouge scores. Exact metrics and comparisons should be checked against the paper's tables and figures before making deployment decisions.

### Limitations and conclusion

Limitation-related text includes: Expert human evaluation While our approach surpasses state-of-the-art results on our dataset in terms of rouge scores, we recognize the limitations of the rouge framework for evaluating summarization (Conroy and Dang, 2008 ; Cohan and Goharian, 2016 ) . These could be addressed in future work with additional post-processing heuristics such as removing repetitive n-grams as done in (Paulus et al . Where the source did not expose a clear limitation in the extracted text, this review records the gap rather than inferring that no limitation exists.
...
- `Sources inspected`: The local verified full-paper PDF and full-paper HTML for arXiv:1905.05818; canonical arXiv metadata URL; three existing Black Lake DEP entries for related context.
...
- `Batch selection`: 75967 PDF candidates; 894 integrity-valid source units; 865 valid identified IDs; 2084 IDs found in repository/memory dedup scans; 396 duplicate exclusions; 0 reselections; 469 unseen eligible IDs; this item is sample position 106 of 200.
...
- `Temporal boundary`: Public deposition date 2026-08-19; source version is the locally archived arXiv version identified as 1905.05818.
...
|---|---|---|---|---|
| S1 | https://arxiv.org/html/1905.05818 | Full-paper text and section-level review | 2026-08-19 | Primary public locator; source file withheld locally. |
| S2 | https://arxiv.org/pdf/1905.05818 | PDF/source integrity status | 2026-08-19 | Primary public locator; source file withheld locally. |
| S3 | https://arxiv.org/abs/1905.05818 | Canonical metadata and stable identifier | 2026-08-19 | Metadata page only; not treated as the paper document. |
