---
title: "Leaky Language Models - DEP-E"
generated_at: "2026-08-18T20:49:54Z"
run_date: "2026-08-19"
artifact_type: "DEP research artifact"
primary_subject: "Leaky Language Models: Stealing Architecture and Inference Optimizations via Per-Token Timing"
...
source_dep: "Black-Lake-Data/.lake-data/DEP-20260725-Tech Intel 1301"
selection_record: "117 candidates; 40 excluded; 77 eligible; draw 1755784316; zero-based index 55"
expansion_record: "10 retained public locators; draw 536255396; zero-based index 6"
...
# Leaky Language Models - DEP-E

## Source Metadata

| ID | Source | Role | Type | Identifier / Version | URL / Local Path | License / Usage Notes | Access Date | Status |
|---|---|---|---|---|---|---|---|---|
| S1 | Selected source DEP README | Source package boundary | Markdown | DEP-20260725-Tech Intel 1301 | https://github.com/Delphoa-Labs/Black-Lake-Data/blob/main/.lake-data/DEP-20260725-Tech%20Intel%201301/README.md | Repository evidence; public URL | 2026-08-19 | Inspected |
| S2 | Deposited source artifact | Source synthesis | Markdown | daily_research_findings_2026-07-25_1301.md | https://github.com/Delphoa-Labs/Black-Lake-Data/blob/main/.lake-data/DEP-20260725-Tech%20Intel%201301/daily_research_findings_2026-07-25_1301.md | Repository evidence; public URL | 2026-08-19 | Inspected |
| S3 | Leaky Language Models: Stealing Architecture and Inference Optimizations via Per-Token Timing | Selected supporting source | arXiv record | 2607.20723 | https://arxiv.org/abs/2607.20723v1 | Metadata and source-package claims only | 2026-08-19 | Canonical arXiv Atom record inspected |

- Paper/work title: Leaky Language Models: Stealing Architecture and Inference Optimizations via Per-Token Timing
- Authors or producing organization: Sadegh Majidi; Niloofar Mireshghallah; Kazem Taram
- Source platform: arXiv
- Publication date: 2026-07-22
- Revision/version date: 2026-07-22
- Stable identifier: 2607.20723
...
|---|---|---|---|---|---|---|
| E1 | https://github.com/Delphoa-Labs/Black-Lake-Data/blob/main/.lake-data/DEP-20260725-Tech%20Intel%201301/README.md | Source DEP README | DEP identity, inventory, attribution, and package boundaries | Source package identity | High | Repository metadata does not independently validate linked claims. |
| E2 | https://github.com/Delphoa-Labs/Black-Lake-Data/blob/main/.lake-data/DEP-20260725-Tech%20Intel%201301/daily_research_findings_2026-07-25_1301.md | Deposited source synthesis | LeakyLMs demonstrates attacks that infer model architecture and inference optimizations using only remote API token-generation timing. The paper reports detection of speculative decoding and draft context-window size, including a measurement that Google Gemini Flash 2.5 uses speculative decoding with an approximately 128K-token draft context window. It also models modern NVIDIA GPU timing to search architecture space, placing near-correct Llama configurations in the top 10 guesses more than 90% of the time. | Source-package claim and selected-thread context | Medium | Derived source synthesis; not an independent reproduction. |
| E3 | https://arxiv.org/abs/2607.20723v1 | Canonical arXiv metadata | Title, identity, version/date metadata, authors or credited organization, and availability | Primary source identity | High | No code, data, model, benchmark, or experiment was executed. |

## Executive Summary

This artifact expands the selected DEP through Leaky Language Models: Stealing Architecture and Inference Optimizations via Per-Token Timing. The deposited source package reports that LeakyLMs demonstrates attacks that infer model architecture and inference optimizations using only remote API token-generation timing. The paper reports detection of speculative decoding and draft context-window size, including a measurement that Google Gemini Flash 2.5 uses speculative decoding with an approximately 128K-token draft context window. It also models modern NVIDIA GPU timing to search architecture space, placing near-correct Llama configurations in the top 10 guesses more than 90% of the time. This is a source-package claim tied to E2, not an independently reproduced result.

The practical relevance is that Hosted model APIs can leak commercially sensitive architecture, hardware, and serving-strategy details even without logits or privileged telemetry. Timing privacy now matters for model gateways, accelerator scheduling, anti-fingerprinting, and inference-security controls. Canonical source identity and availability were inspected (E3), while implementation, data, model, benchmark, and production behavior remain untested. Confidence is therefore high for provenance, medium for the deposited synthesis, and low for claims that would require independent execution.

## Detailed Summary

### Problem context

The selected thread addresses a research or engineering decision represented in the source package as consequential for downstream systems. The package frames the motivation as follows: Hosted model APIs can leak commercially sensitive architecture, hardware, and serving-strategy details even without logits or privileged telemetry. Timing privacy now matters for model gateways, accelerator scheduling, anti-fingerprinting, and inference-security controls.

### Source-reported contribution

The selected DEP summarizes the contribution this way: LeakyLMs demonstrates attacks that infer model architecture and inference optimizations using only remote API token-generation timing. The paper reports detection of speculative decoding and draft context-window size, including a measurement that Google Gemini Flash 2.5 uses speculative decoding with an approximately 128K-token draft context window. It also models modern NVIDIA GPU timing to search architecture space, placing near-correct Llama configurations in the top 10 guesses more than 90% of the time. The canonical record confirms the work's identity, credited authorship or organization, publication locator, and version/date metadata, but this review does not elevate the deposited summary into independently verified evidence.
...
| C2 | The selected supporting record is Leaky Language Models: Stealing Architecture and Inference Optimizations via Per-Token Timing, credited to Sadegh Majidi; Niloofar Mireshghallah; Kazem Taram. | Source metadata | E3 | Canonical identity and availability were directly checked. | High |
| C3 | Hosted model APIs can leak commercially sensitive architecture, hardware, and serving-strategy details even without logits or privileged telemetry. Timing privacy now matters for model gateways, accelerator scheduling, anti-fingerprinting, and inference-security controls. | Reviewer interpretation grounded in source package | E1-E3 | Useful as a research direction, not proof of deployment readiness. | Medium |

## Methodology

- **Research objective**: Preserve and expand one randomly selected supporting thread from DEP-20260725-Tech Intel 1301 as a schema-complete DEP-E research artifact.
...
### New in this pass: Leaky Language Models: Stealing Architecture and Inference Optimizations via Per-Token Timing

| Item | Type | New evidence inspected | Relevance |
|---|---|---|---|
| [Leaky Language Models: Stealing Architecture and Inference Optimizations via Per-Token Timing](https://arxiv.org/abs/2607.20723v1) | Primary arXiv record | Canonical identity, authorship or credited organization, version/date metadata, categories when available, and public availability | Expands the selected DEP through one cryptographically selected supporting thread. |

### Retained from the selected DEP

| Item | Type | Relevance |
|---|---|---|
| https://arxiv.org/abs/2607.20531 | Retained source-package context | Listed by the selected DEP; not re-opened as the expansion target in this pass. |
| https://arxiv.org/abs/2607.21106 | Retained source-package context | Listed by the selected DEP; not re-opened as the expansion target in this pass. |
| https://arxiv.org/abs/2607.20467 | Retained source-package context | Listed by the selected DEP; not re-opened as the expansion target in this pass. |
| https://arxiv.org/abs/2607.20548 | Retained source-package context | Listed by the selected DEP; not re-opened as the expansion target in this pass. |
| https://arxiv.org/abs/2607.20462 | Retained source-package context | Listed by the selected DEP; not re-opened as the expansion target in this pass. |

## Source References

| ID | Reference | Supports | Access date | Notes |
|---|---|---|---|---|
| R1 | https://github.com/Delphoa-Labs/Black-Lake-Data/blob/main/.lake-data/DEP-20260725-Tech%20Intel%201301/README.md | E1 and selected DEP identity | 2026-08-19 | Live source DEP README inspected. |
| R2 | https://github.com/Delphoa-Labs/Black-Lake-Data/blob/main/.lake-data/DEP-20260725-Tech%20Intel%201301/daily_research_findings_2026-07-25_1301.md | E2 and source-package synthesis | 2026-08-19 | Deposited artifact inspected; claims remain source-qualified. |
| R3 | https://arxiv.org/abs/2607.20723v1 | E3 and selected supporting-source identity | 2026-08-19 | Canonical arXiv Atom record inspected; no source payload redistributed. |
...
- Batch item: 39 of 50.
- Eligibility cutoff: 2026-08-17T20:49:54Z; candidate count 117; excluded count 40; eligible count 77.
- DEP draw: accepted UInt32 1755784316; rejection limit 4294967292; attempts 1; zero-based index 55.
- Supporting draw: pool 10; accepted UInt32 536255396; rejection limit 4294967290; attempts 1; zero-based index 6.
