---
title: "arXiv 2305.08379 - DEP-E"
generated_at: "2026-08-19"
artifact_type: "DEP research artifact"
primary_subject: "Source-grounded review of TESS: Text-to-Text Self-Conditioned Simplex Diffusion."
...
primary_url: "https://arxiv.org/abs/2305.08379"
stable_identifier: "arXiv:2305.08379"
...
# arXiv 2305.08379 - DEP-E

## Source Metadata

| Field | Value |
|---|---|
| Paper title | TESS: Text-to-Text Self-Conditioned Simplex Diffusion |
| Authors | Not available from inspected sources |
| Platform | arXiv |
| arXiv ID | 2305.08379 |
| Revision/publication date | Not available from inspected sources |
| Primary URLs | https://arxiv.org/abs/2305.08379; https://arxiv.org/html/2305.08379; https://arxiv.org/pdf/2305.08379 |
...
|---|---|---|---|---|---|---|
| E1 | https://arxiv.org/html/2305.08379 | Primary paper HTML | Full-paper title, abstract, headings, method/evidence text, and limitations cues | Research object, method, evidence, and reviewer notes | High | Text extraction may omit visual layout and fine-grained table context. |
| E2 | https://arxiv.org/pdf/2305.08379 | Primary paper PDF | PDF integrity gate and source-presence cross-check | Complete-source status | High | No independent reproduction or figure-by-figure audit in this batch pass. |
| E3 | https://arxiv.org/abs/2305.08379 | Canonical metadata record | arXiv identifier and public locator | Stable identity and provenance | High | Abstract page is metadata only and was not treated as the full paper. |

## Executive Summary

The reviewed paper, *TESS: Text-to-Text Self-Conditioned Simplex Diffusion*, addresses a research problem described in its full-paper HTML as follows: Abstract Diffusion models have emerged as a powerful paradigm for generation, obtaining strong performance in various continuous domains. However, applying continuous diffusion models to natural language remains challenging due to its discrete nature and the need for a large number of diffusion steps to generate text, making diffusion-based generation expensive. In this work, we propose Te xt-to-text S elf-conditioned S implex Diffusion ( TESS ), a text diffusion model that is fully non-autoregressive, employs a new form of self-conditioning, and applies the diffusion process on the logit simplex space rather than the learned embedding space. Through extensive experiments on natural language understanding and generation tasks including summarization, text simplification, paraphrase generation, and question generation, we demonstrate that TESS outperforms state-of-the-art non-autoregressive models, requires fewer diffusion steps with minimal drop in performance, and is competitive with pretrained autoregressive sequence-to-sequence models. We publicly release our codebase. 1 1 1 https://github.com/allenai/tess-diffusion The source presents the paper's contribution through the following visible section structure: TESS: Text-to-Text Self-Conditioned Simplex Diffusion, Abstract, 1 Introduction, 2 Background, Training, Noise schedule, Inference, 3 Method, Continuous data representation, Time step embeddings, Text-to-text non-autoregressive modeling, Forward diffusion. Reviewer interpretation: the work is most useful as a source-grounded design or evaluation reference, while the exact strength of its claims depends on the experiments, datasets, baselines, and boundary conditions reported in the paper. Full-paper source integrity passed; independent reproduction was not performed.

## Detailed Summary

### Problem and contribution

The paper frames its problem and motivation in the primary source. The extracted abstract is: Abstract Diffusion models have emerged as a powerful paradigm for generation, obtaining strong performance in various continuous domains. However, applying continuous diffusion models to natural language remains challenging due to its discrete nature and the need for a large number of diffusion steps to generate text, making diffusion-based generation expensive. In this work, we propose Te xt-to-text S elf-conditioned S implex Diffusion ( TESS ), a text diffusion model that is fully non-autoregressive, employs a new form of self-conditioning, and applies the diffusion process on the logit simplex space rather than the learned embedding space. Through extensive experiments on natural language understanding and generation tasks including summarization, text simplification, paraphrase generation, and question generation, we demonstrate that TESS outperforms state-of-the-art non-autoregressive models, requires fewer diffusion steps with minimal drop in performance, and is competitive with pretrained autoregressive sequence-to-sequence models. We publicly release our codebase. 1 1 1 https://github.com/allenai/tess-diffusion

### Method and mechanism

The full-paper text contains the following method-related evidence: TESS: Text-to-Text Self-Conditioned Simplex Diffusion 1 Introduction 2 Background Training Noise schedule Inference 3 Method Continuous data representation Time step embeddings Text-to-text non-autoregressive modeling Forward diffusion Training Sampling Self-conditioning Variable sequence length 4 Experiments 4.1 Tasks and Datasets Paraphrase generation Text simplification Question generation Summarization... The available heading structure is retained as a navigation aid rather than treated as proof of every technical claim.

### Evidence and results

Evidence-related text includes: TESS: Text-to-Text Self-Conditioned Simplex Diffusion 1 Introduction 2 Background Training Noise schedule Inference 3 Method Continuous data representation Time step embeddings Text-to-text non-autoregressive modeling Forward diffusion Training Sampling Self-conditioning Variable sequence length 4 Experiments 4.1 Tasks and Datasets Paraphrase generation Text simplification Question generation Summarization... Exact metrics and comparisons should be checked against the paper's tables and figures before making deployment decisions.

### Limitations and conclusion

Limitation-related text includes: In this work, we present TESS , a text-to-text diffusion model, which overcomes several limitations of prior works: restrictions on scale hoogeboom2021argmax ; austin2021structured , dependence on pretrained embeddings strudel2022self , semi-autoregressive nature han2022ssd , and short generation length gong2022diffuseq . Where the source did not expose a clear limitation in the extracted text, this review records the gap rather than inferring that no limitation exists.
...
- `Sources inspected`: The local verified full-paper PDF and full-paper HTML for arXiv:2305.08379; canonical arXiv metadata URL; three existing Black Lake DEP entries for related context.
...
- `Batch selection`: 75967 PDF candidates; 894 integrity-valid source units; 865 valid identified IDs; 2084 IDs found in repository/memory dedup scans; 396 duplicate exclusions; 0 reselections; 469 unseen eligible IDs; this item is sample position 149 of 200.
...
- `Temporal boundary`: Public deposition date 2026-08-19; source version is the locally archived arXiv version identified as 2305.08379.
...
|---|---|---|---|
| DEP-A-20260716-Performance Context | .lake-data/DEP-A/DEP-A-20260716-Performance Context | Shared terms: text, performance | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260716-Performance%20Context/README.md |
...
|---|---|---|---|---|
| S1 | https://arxiv.org/html/2305.08379 | Full-paper text and section-level review | 2026-08-19 | Primary public locator; source file withheld locally. |
| S2 | https://arxiv.org/pdf/2305.08379 | PDF/source integrity status | 2026-08-19 | Primary public locator; source file withheld locally. |
| S3 | https://arxiv.org/abs/2305.08379 | Canonical metadata and stable identifier | 2026-08-19 | Metadata page only; not treated as the paper document. |
| S4 | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260716-Performance%20Context/README.md | Related DEP context: text, performance | 2026-08-19 | Repository file used for conceptual context. |
...
- https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260716-Performance%20Context/README.md — repository context entry `.lake-data/DEP-A/DEP-A-20260716-Performance Context`; selected for conceptual overlap indicated by text, performance; not used as primary evidence.
