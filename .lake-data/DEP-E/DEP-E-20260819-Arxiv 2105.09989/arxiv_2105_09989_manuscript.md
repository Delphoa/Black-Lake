---
title: "arXiv 2105.09989 - DEP-E"
generated_at: "2026-08-19"
artifact_type: "DEP research artifact"
primary_subject: "Source-grounded review of [2105.09989] Multi-group Agnostic PAC Learnability."
...
primary_url: "https://arxiv.org/abs/2105.09989"
stable_identifier: "arXiv:2105.09989"
...
# arXiv 2105.09989 - DEP-E

## Source Metadata

| Field | Value |
|---|---|
| Paper title | [2105.09989] Multi-group Agnostic PAC Learnability |
| Authors | Not available from inspected sources |
| Platform | arXiv |
| arXiv ID | 2105.09989 |
| Revision/publication date | Not available from inspected sources |
| Primary URLs | https://arxiv.org/abs/2105.09989; https://arxiv.org/html/2105.09989; https://arxiv.org/pdf/2105.09989 |
...
|---|---|---|---|---|---|---|
| E1 | https://arxiv.org/html/2105.09989 | Primary paper HTML | Full-paper title, abstract, headings, method/evidence text, and limitations cues | Research object, method, evidence, and reviewer notes | High | Text extraction may omit visual layout and fine-grained table context. |
| E2 | https://arxiv.org/pdf/2105.09989 | Primary paper PDF | PDF integrity gate and source-presence cross-check | Complete-source status | High | No independent reproduction or figure-by-figure audit in this batch pass. |
| E3 | https://arxiv.org/abs/2105.09989 | Canonical metadata record | arXiv identifier and public locator | Stable identity and provenance | High | Abstract page is metadata only and was not treated as the full paper. |

## Executive Summary

The reviewed paper, *[2105.09989] Multi-group Agnostic PAC Learnability*, addresses a research problem described in its full-paper HTML as follows: Abstract An agnostic PAC learning algorithm finds a predictor that is competitive with the best predictor in a benchmark hypothesis class, where competitiveness is measured with respect to a given loss function. However, its predictions might be quite sub-optimal for structured subgroups of individuals, such as protected demographic groups. Motivated by such fairness concerns, we study “multi-group agnostic PAC learnability”: fixing a measure of loss, a benchmark class ℋ ℋ \mathcal{H} and a (potentially) rich collection of subgroups 𝒢 𝒢 \mathcal{G} , the objective is to learn a single predictor such that the loss experienced by every group g ∈ 𝒢 𝑔 𝒢 g\in\mathcal{G} is not much larger than the best possible loss for this group within ℋ ℋ \mathcal{H} . Under natural conditions, we provide a characterization of the loss functions for which such a predictor is guaranteed to exist. For any such loss function we construct a learning algorithm whose sample complexity is logarithmic in the size of the collection 𝒢 𝒢 \mathcal{G} . Our results unify and extend previous positive and negative results from the multi-group fairness literature, which applied for specific loss functions. The source presents the paper's contribution through the following visible section structure: Multi-group Agnostic PAC Learnability, Abstract, 1 Introduction, 1.1 Our contributions, 1.2 Related work, Discussion., Organization., 2 Preliminaries, 2.1 General loss functions, Definition 2.1 (Decomposable losses) ., Beyond decomposable losses., Definition 2.2 (Unambiguity) .. Reviewer interpretation: the work is most useful as a source-grounded design or evaluation reference, while the exact strength of its claims depends on the experiments, datasets, baselines, and boundary conditions reported in the paper. Full-paper source integrity passed; independent reproduction was not performed.

## Detailed Summary

### Problem and contribution

The paper frames its problem and motivation in the primary source. The extracted abstract is: Abstract An agnostic PAC learning algorithm finds a predictor that is competitive with the best predictor in a benchmark hypothesis class, where competitiveness is measured with respect to a given loss function. However, its predictions might be quite sub-optimal for structured subgroups of individuals, such as protected demographic groups. Motivated by such fairness concerns, we study “multi-group agnostic PAC learnability”: fixing a measure of loss, a benchmark class ℋ ℋ \mathcal{H} and a (potentially) rich collection of subgroups 𝒢 𝒢 \mathcal{G} , the objective is to learn a single predictor such that the loss experienced by every group g ∈ 𝒢 𝑔 𝒢 g\in\mathcal{G} is not much larger than the best possible loss for this group within ℋ ℋ \mathcal{H} . Under natural conditions, we provide a characterization of the loss functions for which such a predictor is guaranteed to exist. For any such loss function we construct a learning algorithm whose sample complexity is logarithmic in the size of the collection 𝒢 𝒢 \mathcal{G} . Our results unify and extend previous positive and negative results from the multi-group fairness literature, which applied for specific loss functions.

### Method and mechanism

The full-paper text contains the following method-related evidence: 819702), from the Israel Science Foundation (grant number 5219/17), from the U.S.-Israel Binational Science Foundation (grant 2018102), and from the Simons Foundation Collaboration on the Theory of Algorithmic Fairness. 819702), from the Israel Science Foundation (grant number 5219/17) and from the Simons Foundation Collaboration on the Theory of Algorithmic Fairness. The available heading structure is retained as a navigation aid rather than treated as proof of every technical claim.

### Evidence and results

Evidence-related text includes: Abstract An agnostic PAC learning algorithm finds a predictor that is competitive with the best predictor in a benchmark hypothesis class, where competitiveness is measured with respect to a given loss function. Motivated by such fairness concerns, we study “multi-group agnostic PAC learnability”: fixing a measure of loss, a benchmark class ℋ ℋ \mathcal{H} and a (potentially) rich collection of subgroups 𝒢 𝒢... Exact metrics and comparisons should be checked against the paper's tables and figures before making deployment decisions.

### Limitations and conclusion

Limitation-related text includes: Typically we are interested in these losses as either an additional constraint on an existing objective, or paired with an additional loss (e.g. Algorithm 1 𝙼𝚞𝚕𝚝𝚒𝙶𝚛𝚘𝚞𝚙 𝙻 , 𝚏 ​ ( ϵ , δ , γ , ℋ , 𝒢 ) subscript 𝙼𝚞𝚕𝚝𝚒𝙶𝚛𝚘𝚞𝚙 𝙻 𝚏 italic-ϵ 𝛿 𝛾 ℋ 𝒢 \mathtt{MultiGroup_{L,f}}(\epsilon,\delta,\gamma,\mathcal{H},\mathcal{G}) 1: Parameters: loss function L 𝐿 L , function f : 𝒳 × [ 0 , 1 ] → [ 0 , 1 ] : 𝑓 → 𝒳 0 1 0 1... Where the source did not expose a clear limitation in the extracted text, this review records the gap rather than inferring that no limitation exists.
...
- `Sources inspected`: The local verified full-paper PDF and full-paper HTML for arXiv:2105.09989; canonical arXiv metadata URL; three existing Black Lake DEP entries for related context.
...
- `Batch selection`: 75967 PDF candidates; 894 integrity-valid source units; 865 valid identified IDs; 2084 IDs found in repository/memory dedup scans; 396 duplicate exclusions; 0 reselections; 469 unseen eligible IDs; this item is sample position 172 of 200.
...
- `Temporal boundary`: Public deposition date 2026-08-19; source version is the locally archived arXiv version identified as 2105.09989.
...
| DEP-A-20260719-Oracle Agent Memory | .lake-data/DEP-A/DEP-A-20260719-Oracle Agent Memory | Shared terms: agent, memory | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260719-Oracle%20Agent%20Memory/README.md |
...
|---|---|---|---|---|
| S1 | https://arxiv.org/html/2105.09989 | Full-paper text and section-level review | 2026-08-19 | Primary public locator; source file withheld locally. |
| S2 | https://arxiv.org/pdf/2105.09989 | PDF/source integrity status | 2026-08-19 | Primary public locator; source file withheld locally. |
| S3 | https://arxiv.org/abs/2105.09989 | Canonical metadata and stable identifier | 2026-08-19 | Metadata page only; not treated as the paper document. |
...
| S6 | https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260719-Oracle%20Agent%20Memory/README.md | Related DEP context: agent, memory | 2026-08-19 | Repository file used for conceptual context. |
...
- https://github.com/Delphoa/Black-Lake/blob/main/.lake-data/DEP-A/DEP-A-20260719-Oracle%20Agent%20Memory/README.md — repository context entry `.lake-data/DEP-A/DEP-A-20260719-Oracle Agent Memory`; selected for conceptual overlap indicated by agent, memory; not used as primary evidence.
