---
title: "arXiv 2005.02365 - DEP-E"
generated_at: "2026-08-19"
artifact_type: "DEP research artifact"
primary_subject: "Source-grounded review of [2005.02365] SLEDGE: A Simple Yet Effective Baseline for COVID-19 Scientific Knowledge Search."
...
primary_url: "https://arxiv.org/abs/2005.02365"
stable_identifier: "arXiv:2005.02365"
...
# arXiv 2005.02365 - DEP-E

## Source Metadata

| Field | Value |
|---|---|
| Paper title | [2005.02365] SLEDGE: A Simple Yet Effective Baseline for COVID-19 Scientific Knowledge Search |
| Authors | Not available from inspected sources |
| Platform | arXiv |
| arXiv ID | 2005.02365 |
| Revision/publication date | Not available from inspected sources |
| Primary URLs | https://arxiv.org/abs/2005.02365; https://arxiv.org/html/2005.02365; https://arxiv.org/pdf/2005.02365 |
...
|---|---|---|---|---|---|---|
| E1 | https://arxiv.org/html/2005.02365 | Primary paper HTML | Full-paper title, abstract, headings, method/evidence text, and limitations cues | Research object, method, evidence, and reviewer notes | High | Text extraction may omit visual layout and fine-grained table context. |
| E2 | https://arxiv.org/pdf/2005.02365 | Primary paper PDF | PDF integrity gate and source-presence cross-check | Complete-source status | High | No independent reproduction or figure-by-figure audit in this batch pass. |
| E3 | https://arxiv.org/abs/2005.02365 | Canonical metadata record | arXiv identifier and public locator | Stable identity and provenance | High | Abstract page is metadata only and was not treated as the full paper. |

## Executive Summary

The reviewed paper, *[2005.02365] SLEDGE: A Simple Yet Effective Baseline for COVID-19 Scientific Knowledge Search*, addresses a research problem described in its full-paper HTML as follows: Abstract With worldwide concerns surrounding the Severe Acute Respiratory Syndrome Coronavirus 2 (SARS-CoV-2), there is a rapidly growing body of literature on the virus. Clinicians, researchers, and policy-makers need a way to effectively search these articles. In this work, we present a search system called Sledge , which utilizes SciBERT to effectively re-rank articles. We train the model on a general-domain answer ranking dataset, and transfer the relevance signals to SARS-CoV-2 for evaluation. We observe Sledge ’s effectiveness as a strong baseline on the TREC-COVID challenge (topping the learderboard with an nDCG@10 of 0.6844). Insights provided by a detailed analysis provide some potential future directions to explore, including the importance of filtering by date and the potential of neural methods that rely more heavily on count signals. We release the code to facilitate future work on this critical task. 1 1 1 https://github.com/Georgetown-IR-Lab/covid-neural-ir The source presents the paper's contribution through the following visible section structure: SLEDGE: A Simple Yet Effective Baseline for COVID-19 S cientific Know ledge Search, Abstract, 1 Introduction, 2 Related Work, 3 Sledge, 3.1 First-Stage Retrieval, 3.2 Neural Re-Ranking, 4 Experimental setup, Note on manual vs automatic runs, 5 Results, 5.1 Ranking with light hyperparmeter tuning, 5.2 Ranking without hyperparameter tuning. Reviewer interpretation: the work is most useful as a source-grounded design or evaluation reference, while the exact strength of its claims depends on the experiments, datasets, baselines, and boundary conditions reported in the paper. Full-paper source integrity passed; independent reproduction was not performed.

## Detailed Summary

### Problem and contribution

The paper frames its problem and motivation in the primary source. The extracted abstract is: Abstract With worldwide concerns surrounding the Severe Acute Respiratory Syndrome Coronavirus 2 (SARS-CoV-2), there is a rapidly growing body of literature on the virus. Clinicians, researchers, and policy-makers need a way to effectively search these articles. In this work, we present a search system called Sledge , which utilizes SciBERT to effectively re-rank articles. We train the model on a general-domain answer ranking dataset, and transfer the relevance signals to SARS-CoV-2 for evaluation. We observe Sledge ’s effectiveness as a strong baseline on the TREC-COVID challenge (topping the learderboard with an nDCG@10 of 0.6844). Insights provided by a detailed analysis provide some potential future directions to explore, including the importance of filtering by date and the potential of neural methods that rely more heavily on count signals. We release the code to facilitate future work on this critical task. 1 1 1 https://github.com/Georgetown-IR-Lab/covid-neural-ir

### Method and mechanism

The full-paper text contains the following method-related evidence: Insights provided by a detailed analysis provide some potential future directions to explore, including the importance of filtering by date and the potential of neural methods that rely more heavily on count signals. Since at the time of writing there is no available training data for COVID-19 related search, we additionally use a domain transfer approach by training Sledge on MS-MARCO Campos et al. The available heading structure is retained as a navigation aid rather than treated as proof of every technical claim.

### Evidence and results

Evidence-related text includes: We train the model on a general-domain answer ranking dataset, and transfer the relevance signals to SARS-CoV-2 for evaluation. This results in a considerable burden for those seeking information about various facets of the virus, including researchers, clinicians, and policy-makers. Exact metrics and comparisons should be checked against the paper's tables and figures before making deployment decisions.

### Limitations and conclusion

Limitation-related text includes: We release the code to facilitate future work on this critical task. Insights from the analysis highlight the importance of a date filter for improving precision, and the possible benefit of utilizing models that include count-based signals in future work. Where the source did not expose a clear limitation in the extracted text, this review records the gap rather than inferring that no limitation exists.
...
- `Sources inspected`: The local verified full-paper PDF and full-paper HTML for arXiv:2005.02365; canonical arXiv metadata URL; three existing Black Lake DEP entries for related context.
...
- `Batch selection`: 75967 PDF candidates; 894 integrity-valid source units; 865 valid identified IDs; 2084 IDs found in repository/memory dedup scans; 396 duplicate exclusions; 0 reselections; 469 unseen eligible IDs; this item is sample position 98 of 200.
...
- `Temporal boundary`: Public deposition date 2026-08-19; source version is the locally archived arXiv version identified as 2005.02365.
...
|---|---|---|---|---|
| S1 | https://arxiv.org/html/2005.02365 | Full-paper text and section-level review | 2026-08-19 | Primary public locator; source file withheld locally. |
| S2 | https://arxiv.org/pdf/2005.02365 | PDF/source integrity status | 2026-08-19 | Primary public locator; source file withheld locally. |
| S3 | https://arxiv.org/abs/2005.02365 | Canonical metadata and stable identifier | 2026-08-19 | Metadata page only; not treated as the paper document. |
