# Preserved failed review record — arXiv:2605.14494v1
...
Original failure codes: `table_result_missing_real_header_or_value`, `table_result_missing_real_header_or_value`, `table_result_missing_real_header_or_value`, `table_result_missing_real_header_or_value`, `table_result_missing_real_header_or_value`, `table_result_missing_real_header_or_value`, `table_result_missing_real_header_or_value`, `table_result_missing_real_header_or_value`, `placeholder_scaffolding_present`, `verified_external_context_missing`, `verified_external_context_missing`.
...
Reviewer inference: the durable mechanism is the source-defined change centered on scenario, encoder, graph, and embeddings, rather than the paper's brand name. This interpretation predicts that a matched intervention on scenario changes Instances; it is not an author claim or theorem. *(paper-specific reconstruction; source locator retained in the private substantive profile).* The source's technical path is anchored to 4 Methodology, 4.2.1 Model Architecture. The review treats those sections as the authority for inputs, state, transformations, objectives, and outputs; heading labels alone are not promoted into performance claims. This reconstruction does not replace the authors' definitions and does not imply that a heading or architecture label proves an empirical claim.
...
**Formal object 1 at 3 Preliminaries — The expression encodes definition or equality within 3 Preliminaries; its semantic role remains bound to that section..** `\Xi=\{\xi_{1},\dots,\xi_{S}\}`
Variables: "S".
...
Source locator: private full-paper evidence dossier for arXiv:2605.14494, 3 Preliminaries.

**Formal object 2 at 3 Preliminaries — The expression encodes optimization or selection within 3 Preliminaries; its semantic role remains bound to that section..** `\displaystyle\min_{x\in X}`
Variables: "x".
...
**Formal object 3 at 3 Preliminaries — The expression encodes optimization or selection within 3 Preliminaries; its semantic role remains bound to that section..** `\displaystyle c^{\top}x+\max_{\xi\in\Xi}\min_{y\in F(x,\xi)}b_{\xi}^{\top}y,`
Variables: "c, x, y, F, and b_{\\xi}^{\\top}y".
Sign/normalization/conditioning/surrogate audit: "Detected operations: optimization or selection. The review must retain the displayed sign, denominator, conditioning, and surrogate boundary; code behavior was not independently checked.".
Source locator: private full-paper evidence dossier for arXiv:2605.14494, 3 Preliminaries.

**Formal object 4 at 3 Preliminaries — The expression encodes a paper-defined formal relation within 3 Preliminaries; its semantic role remains bound to that section..** `x\in\mathbb{R}^{n}`
Variables: "x, R, and n".
...
**Formal object 5 at 3 Preliminaries — The expression encodes a paper-defined formal relation within 3 Preliminaries; its semantic role remains bound to that section..** `y\in\mathbb{R}^{m}`
Variables: "y, R, and m".
Sign/normalization/conditioning/surrogate audit: "Detected operations: none mechanically classified. The review must retain the displayed sign, denominator, conditioning, and surrogate boundary; code behavior was not independently checked.".
Source locator: private full-paper evidence dossier for arXiv:2605.14494, 3 Preliminaries.

**Formal object 6 at 3 Preliminaries — The expression encodes a paper-defined formal relation within 3 Preliminaries; its semantic role remains bound to that section..** `c\in\mathbb{R}^{n}`
Variables: "c, R, and n".
Sign/normalization/conditioning/surrogate audit: "Detected operations: none mechanically classified. The review must retain the displayed sign, denominator, conditioning, and surrogate boundary; code behavior was not independently checked.".
Source locator: private full-paper evidence dossier for arXiv:2605.14494, 3 Preliminaries.
...
2. At 5 Experiments, the paper reports a training-related operation involving Experiments, Gurobi, conducted, server, EPYC, and Core. *(source locator: private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments)*
3. At 5 Experiments, the paper reports a training-related operation involving Training, validation, instances, test, Experiments, and instance. *(source locator: private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments)*
4. At 5.1 Comparison Analysis, the paper reports a training-related operation involving NeurPRISE, across, regret, PRISE, achieves, and budgets. *(source locator: private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis)*
...
2. At 5 Experiments, the paper reports an inference or deployment action involving instances, Uniform, Experiments, Instance, Generation, and generate. *(source locator: private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments)*
3. At 5.1 Comparison Analysis, the paper reports an inference or deployment action involving NeurPRISE, across, regret, PRISE, achieves, and budgets. *(source locator: private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis)*
4. At G.4 Feasibility Analysis, the paper reports an inference or deployment action involving recourse, demand, scenarios, infeasibility, Feasibility, and uncertainty. *(source locator: private full-paper evidence dossier for arXiv:2605.14494, G.4 Feasibility Analysis)*
...
The documented dataflow is reconstructed only across 4 Methodology, and 4.2.1 Model Architecture, where the source associates scenario, encoder, graph, embeddings, SEquential, PRISE, and reduction. Exact sequencing, hidden preprocessing, and post-processing remain bounded to the cited sections.
...
|---|---|---|---|
| AdamW | Source mentions Training, validation, test, and train partitions; exact counts and leakage controls require the full-section audit. | Source-linked operations: none resolved from the bounded excerpt; no unreported preprocessing is inferred. | private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments |
| set | Source mentions Training, validation, test, and train partitions; exact counts and leakage controls require the full-section audit. | Source-linked operations: none resolved from the bounded excerpt; no unreported preprocessing is inferred. | private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments |
| E.2 | Source mentions Training, validation, test, and train partitions; exact counts and leakage controls require the full-section audit. | Source-linked operations: none resolved from the bounded excerpt; no unreported preprocessing is inferred. | private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments |
| wall-clock | Source mentions evaluation partitions; exact counts and leakage controls require the full-section audit. | Source-linked operations: none resolved from the bounded excerpt; no unreported preprocessing is inferred. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
| scenario-reduction | Source mentions evaluation partitions; exact counts and leakage controls require the full-section audit. | Source-linked operations: none resolved from the bounded excerpt; no unreported preprocessing is inferred. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
| PRISE | Source mentions evaluation partitions; exact counts and leakage controls require the full-section audit. | Source-linked operations: none resolved from the bounded excerpt; no unreported preprocessing is inferred. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
| NeurPRISE | Source mentions evaluation partitions; exact counts and leakage controls require the full-section audit. | Source-linked operations: none resolved from the bounded excerpt; no unreported preprocessing is inferred. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
| MILP | Source mentions evaluation partitions; exact counts and leakage controls require the full-section audit. | Source-linked operations: none resolved from the bounded excerpt; no unreported preprocessing is inferred. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
...
|---|---|---|---|
| SEL | Paper-reported comparator at 5 Experiments; official implementation or copied-result status is not inferred from the name. | Comparable model, data, tuning, hardware, and compute budgets require direct source or implementation receipts. | private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments |
| minimum-cost | Paper-reported comparator at 5 Experiments; official implementation or copied-result status is not inferred from the name. | Comparable model, data, tuning, hardware, and compute budgets require direct source or implementation receipts. | private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments |
| subset | Paper-reported comparator at 5 Experiments; official implementation or copied-result status is not inferred from the name. | Comparable model, data, tuning, hardware, and compute budgets require direct source or implementation receipts. | private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments |
| CFLP | Paper-reported comparator at 5 Experiments; official implementation or copied-result status is not inferred from the name. | Comparable model, data, tuning, hardware, and compute budgets require direct source or implementation receipts. | private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments |
| wall-clock | Paper-reported comparator at 5.1 Comparison Analysis; official implementation or copied-result status is not inferred from the name. | Comparable model, data, tuning, hardware, and compute budgets require direct source or implementation receipts. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
| scenario-reduction | Paper-reported comparator at 5.1 Comparison Analysis; official implementation or copied-result status is not inferred from the name. | Comparable model, data, tuning, hardware, and compute budgets require direct source or implementation receipts. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
| PRISE | Paper-reported comparator at 5.1 Comparison Analysis; official implementation or copied-result status is not inferred from the name. | Comparable model, data, tuning, hardware, and compute budgets require direct source or implementation receipts. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
| NeurPRISE | Paper-reported comparator at 5.1 Comparison Analysis; official implementation or copied-result status is not inferred from the name. | Comparable model, data, tuning, hardware, and compute budgets require direct source or implementation receipts. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
...
|---|---|---|---|
| loss | Paper-defined evaluation unit at 5 Experiments; exclusions and conditioning were not mechanically resolved | lower is normally better; confirm the paper's sign and aggregation | private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments |
...
|---|---|---|---|---|---|---|---|
| wall-clock | Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty | comparator retained at the cited source locator; not inferred | configuration retained at the cited source locator | source-defined metric | 1, 2, 4, and 6 | Paper-reported numeric evidence only; denominator, conditioning, uncertainty, and table/prose consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
| evaluation object at 5.1 Comparison Analysis | Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty | comparator retained at the cited source locator; not inferred | configuration retained at the cited source locator | source-defined metric | 1, and 2 | Paper-reported numeric evidence only; denominator, conditioning, uncertainty, and table/prose consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
| SEL | Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty | NeurPRISE | configuration retained at the cited source locator | source-defined metric | 1, and 2 | Paper-reported numeric evidence only; denominator, conditioning, uncertainty, and table/prose consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
| NeurPRISE | Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty | scenario-reduction | configuration retained at the cited source locator | source-defined metric | 2, and 4 | Paper-reported numeric evidence only; denominator, conditioning, uncertainty, and table/prose consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, 5.1 Comparison Analysis |
...
- No independent reproduction was performed for arXiv:2605.14494v1; Instances, across, NeurPRISE, and scenarios remains paper-reported. *(source locator: private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments, and 5.1 Comparison Analysis)*
...
|---|---|---|---|
| Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty: paper-reported problem claim centered on Scenario, uncertainty, PRISE, and Optimization | Located at Abstract; extracted numeric markers: 3, 1, 9, and 2 (private full-paper evidence dossier for arXiv:2605.14494, Abstract) | promising but bounded to the paper's stated task | The review does not expand this claim to untested data, scales, budgets, hardware, populations, or deployment conditions. |
| Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty: paper-reported mechanism claim centered on scenario, encoder, graph, and embeddings | Located at 4 Methodology; extracted numeric markers: 1, 16, and 3 (private full-paper evidence dossier for arXiv:2605.14494, 4 Methodology) | paper-reported mechanism; causal attribution requires matched ablation | The review does not expand this claim to untested data, scales, budgets, hardware, populations, or deployment conditions. |
| Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty: paper-reported result claim centered on Instances, across, NeurPRISE, and scenarios | Located at 5 Experiments; extracted numeric markers: 7543, 32, 2.80, and 48 (private full-paper evidence dossier for arXiv:2605.14494, 5 Experiments) | supported only under reported evaluation conditions | The review does not expand this claim to untested data, scales, budgets, hardware, populations, or deployment conditions. |
...
- **Predecessor/prior work (blocked):** The bounded online record did not verify a noncanonical predecessor or prior-work source URL; the corresponding field-level claim remains unvalidated. *(evidence locator: online-vetting check for arXiv:2605.14494)*
- **Alternative or benchmark (blocked):** The bounded online record did not verify a noncanonical alternative or benchmark source URL; the corresponding field-level claim remains unvalidated. *(evidence locator: online-vetting check for arXiv:2605.14494)*
- **Code/data (blocked):** The bounded online record did not verify a noncanonical code or data artifact URL; the corresponding field-level claim remains unvalidated. *(evidence locator: online-vetting check for arXiv:2605.14494)*
...
Reviewer inference: the durable mechanism is the source-defined change centered on scenario, encoder, graph, and embeddings, rather than the paper's brand name. This interpretation predicts that a matched intervention on scenario changes Instances; it is not an author claim or theorem.
...
Substantive evidence boundary: The profile binds arXiv:2605.14494v1 to a complete local PDF and full-paper HTML, 61 headings, 23 tables, 8 figures, and 371 extracted mathematical objects, and 0 bounded verified noncanonical or canonical URLs. No experiment, code path, data pipeline, hardware measurement, security property, or production claim was independently reproduced.
...
**Proposition:** Reviewer hypothesis: the source-linked scenario operation is causally responsible for part of the reported Instances behavior.
**Predicted observation:** Removing or neutralizing scenario under matched data and compute will measurably weaken Instances.
**Falsifying observation:** A competent matched control without scenario preserves the same Instances distribution within uncertainty.
...
**Proposition:** Reviewer hypothesis: the relation between scenario, and encoder and Instances, and across weakens under an untested scale, distribution, or budget shift.
...
> Durably, Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty should be remembered as a tested relation between scenario, encoder, and graph and Instances, across, and NeurPRISE under the configurations at 5 Experiments, and 5.1 Comparison Analysis, not as proof outside those conditions.
...
|---|---|---|
| Table 1 | Purpose: The caption frames this object around instances, Performance, comparison, small-scale, test, and dash.; result: Paper-reported numeric markers: 1, and 250; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 1 caption |
| Table 2 | Purpose: The caption frames this object around Medium, large-scale, comparison, test, instances, and notation.; result: Paper-reported numeric markers: 2, 50, and 1; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 2 caption |
| Table 3 | Purpose: The caption frames this object around Generalization, times, problem, and size.; result: Paper-reported numeric markers: 3, 5 ×, and 5; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 3 caption |
| Table 4 | Purpose: The caption frames this object around Scenario, and generalization.; result: Paper-reported numeric markers: 4, 50, 100, and 200; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 4 caption |
| Table 5 | Purpose: The caption frames this object around Distribution, shift, and Normal.; result: Paper-reported numeric markers: 5; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 5 caption |
| Table 6 | Purpose: The caption frames this object around Distribution, and shift.; result: Paper-reported numeric markers: 6; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 6 caption |
| Table 7 | Purpose: The caption frames this object around Compression, ratio, budget, denotes, percentage, and instances.; result: Paper-reported numeric markers: 7, and 8; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 7 caption |
| Table 8 | Purpose: The caption frames this object around Node, edge, features, encoder, problem, and class.; result: Paper-reported numeric markers: 8; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 8 caption |
| Table 9 | Purpose: The caption frames this object around Default, NeurPRISE, and hyperparameters.; result: Paper-reported numeric markers: 9; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 9 caption |
| Table 10 | Purpose: The caption frames this object around Small-scale, versus, target-scale, training, and comparison.; result: Paper-reported numeric markers: 10; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 10 caption |
| Table 11 | Purpose: The caption frames this object around Effect, training-set, size, in-distribution, generalization, and performance.; result: Paper-reported numeric markers: 11; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 11 caption |
| Table 12 | Purpose: The caption frames this object around Seed, variance, analysis, NeurPRISE, across, and five.; result: Paper-reported numeric markers: 12; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 12 caption |
| Table 13 | Purpose: The caption frames this object around Time-budgeted, comparison, receive, per-instance, wall-clock, and budget.; result: Paper-reported numeric markers: 13, and 3; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 13 caption |
| Table 14 | Purpose: The caption frames this object around Medium, and instances.; result: Paper-reported numeric markers: 14; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 14 caption |
| Table 15 | Purpose: The caption frames this object around Large, and instances.; result: Paper-reported numeric markers: 15; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 15 caption |
| Table 16 | Purpose: The caption frames this object around Encoder-fusion, ablation, small, instances, averaged, and seeds.; result: Paper-reported numeric markers: 16, and 5; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 16 caption |
| Table 17 | Purpose: The caption frames this object around Loss-function, and ablation.; result: Paper-reported numeric markers: 17; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 17 caption |
| Table 18 | Purpose: The caption frames this object around CFLP, graph, encoding, and features.; result: Paper-reported numeric markers: 18; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 18 caption |
| Table 19 | Purpose: The caption frames this object around Encoding, ablation, CFLP, small, scale, and Result.; result: Paper-reported numeric markers: 19, and 6; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 19 caption |
| Table 20 | Purpose: The caption frames this object around CFLP, performance, across, three, scales, and infeasible.; result: Paper-reported numeric markers: 20, 6, and 75%; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 20 caption |
| Table 21 | Purpose: The caption frames this object around CFLP, and generalization.; result: Paper-reported numeric markers: 21; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 21 caption |
| Table 22 | Purpose: The caption frames this object around CFLP, distribution, and shift.; result: Paper-reported numeric markers: 22, and 6; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 22 caption |
| Table 23 | Purpose: The caption frames this object around CFLP, infeasibility, and rate.; result: Paper-reported numeric markers: 23; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Table 23 caption |
| Figure 1 | Purpose: The caption frames this object around Right, NeurPRISE, illustrative, overview, Left, and PRISE.; result: Paper-reported numeric markers: 1; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Figure 1 caption |
| Algorithm 1 | Purpose: The caption frames this object around Algorithm, and PRISE.; result: Paper-reported numeric markers: 1; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Algorithm 1 caption |
| Figure inventory item 3 | Purpose: The caption frames this object around Mean, marginal, gain, across, and iterations.; result: Paper-reported numeric markers: none mechanically isolated; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Figure inventory item 3 caption |
| Figure inventory item 4 | Purpose: The caption frames this object around Distribution, PRISE, stop, and steps.; result: Paper-reported numeric markers: none mechanically isolated; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Figure inventory item 4 caption |
| Figure 2 | Purpose: The caption frames this object around PRISE, Empirical, analysis, small-scale, instances, and Marginal.; result: Paper-reported numeric markers: 2; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Figure 2 caption |
| Figure 3 | Purpose: The caption frames this object around form, Constraint, variable, bipartite, graph, and scenario.; result: Paper-reported numeric markers: 3; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Figure 3 caption |
| Figure 4 | Purpose: The caption frames this object around Regret, total, solve, time, instances, and different.; result: Paper-reported numeric markers: 4, and 50; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Figure 4 caption |
| Figure 5 | Purpose: The caption frames this object around Problem-specific, graph, and CFLP.; result: Paper-reported numeric markers: 5; direction and context remain bound to the caption and surrounding section.; caveat: Caption-level disposition only; cells, axes, denominators, conditioning, and appendix consistency were not independently reproduced. | private full-paper evidence dossier for arXiv:2605.14494, Figure 5 caption |
