# Forensic Traces for Agent Memory Poisoning

## A whitepaper-grade source-bundle review and independent re-conceptualization

**Source work:** Forensic Trajectory Signatures for Agent Memory Poisoning Detection.[^paper]
**Source DEP-E:** .lake-data/DEP-E/DEP-E-20260711-Agent Memory Forensics[^dep]
...
This intake reviews Forensic Trajectory Signatures for Agent Memory Poisoning Detection through the complete DEP-E repository record. It asks whether delayed harmful actions caused through explicit persistent-memory retrieval leave operational tool-call signatures that can support defensive detection. The record is coherent enough to reconstruct the mechanism, evidence, limitations, and open work, but it is not an independently reproduced full-paper review.

The reviewed detector uses ordered, timestamped tool-call traces rather than prompt content, memory values, model weights, or hidden activations. Its central invariant is recall-before-send in an architecture where the harmful routing value must be recovered through an explicit memory tool; nineteen structural and count features support simple classifiers.
...
Whether delayed harmful actions caused through explicit persistent-memory retrieval leave operational tool-call signatures that can support defensive detection is important because it joins representation, policy, and observable outcome. A credible review must ask what information enters, how state is formed, what objective shapes behavior, and which metric stands in for the intended effect.
...
The source path is .lake-data/DEP-E/DEP-E-20260711-Agent Memory Forensics at commit a1ffd4737ce95c14f3a15251ee4fbba4403108a5. It contains exactly README.md and memory-forensics.md. Both files were read from beginning to end. The README supplies classification, inventory, public context, insights, and attribution. The manuscript supplies metadata, evidence ledger, detailed reconstruction, claims, method, constraints, observations, proposed extensions, references, and validation notes.

The canonical public locator is https://arxiv.org/abs/2606.30566; a persistent or related locator is https://doi.org/10.48550/arXiv.2606.30566. The complete canonical paper was not independently used to validate all paper-level detail in this run. No source document was uploaded, no source Markdown copied, and the source record remains unchanged.
...
The source report describes 5,040 factorial runs across nine open-source models and seven defense conditions. Logistic regression, random forest, and gradient-boosted trees are evaluated with stratified five-fold cross-validation, leave-one-model-out tests, limited frontier-model probes, and a strict prefix-only variant.
...
The paper reports AUC 0.9563 for the simple invariant, AUC 0.9904 for each full-trajectory classifier with BCa 95% interval 0.987 to 0.993, recall 0.9838, and prefix-only AUC 0.934. These results use poisoned-but-defended sessions as negatives rather than ordinary benign workloads.
...
The simple invariant, full feature set, prefix restriction, cross-model tests, and implicit-bypass example provide useful mechanism evidence. They also show the signature is channel-specific: implicit retrieval, hidden caches, or prompt-inline attacks need not traverse the logged recall path.
...
The public locator https://arxiv.org/abs/2606.30566 and related identifier https://doi.org/10.48550/arXiv.2606.30566 anchor identity and context.[^paper] They do not independently supply a reproduction. Related work named by the DEP-E is contextual, not confirmation.
...
Archive the method as a privacy-minimized evidence-ledger design rather than a universal attack detector. A normalized event schema should bind every action to operation order, state age, policy, and correlation identifiers while preserving content minimization and calibrated human review.
...
The negative class is not representative benign traffic. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 2. Critical note

Reasoning models may recall benign facts and increase false positives. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 3. Critical note

Unlogged or implicit memory paths evade the evidence model. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 4. Critical note

Telemetry can remain sensitive even without content. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 5. Critical note

No code, raw traces, or attack reproduction occurred. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.
...
1. **Calibrate against genuine benign workloads.** This is a system and evidence implication, not reproduced capability.
2. **Normalize event schemas before model-specific scoring.** This is a system and evidence implication, not reproduced capability.
3. **Treat detection as defense in depth with policy enforcement.** This is a system and evidence implication, not reproduced capability.
4. **Minimize and protect retained telemetry.** This is a system and evidence implication, not reproduced capability.
...
**Proposition:** A normalized trace schema transfers better than raw model-specific features.
...
**Proposition:** Benign-workload calibration materially lowers usable thresholds.
...
**Proposition:** Prefix-only signals retain value but lose attack recall.
...
**Proposition:** Explicit policy gates outperform detection alone on bypass paths.
...
3. Match embedding dimension and tuning budget across Forensic Traces for Agent Memory Poisoning, node2vec, attribute-only, and modern GNN baselines.
...
Challenge assumptions with shifts in data source, scenario complexity, sample composition, resource budget, noise, missingness, and distribution relevant to Forensic Traces for Agent Memory Poisoning. Separate recalibration from retraining and disclose target-domain information. A method that works after extensive target search supports a different claim from zero-shot transfer. Report strata so averages cannot conceal systematic collapse or harm.
...
- Complete authorized paper: https://arxiv.org/abs/2606.30566
- Persistent identifier: https://doi.org/10.48550/arXiv.2606.30566
- Immutable DEP-E record: https://github.com/Delphoa/Black-Lake/tree/a1ffd4737ce95c14f3a15251ee4fbba4403108a5/.lake-data/DEP-E/DEP-E-20260709-Forensic Traces for Agent Memory Poisoning%20Embeddings
...
[^paper]: Forensic Trajectory Signatures for Agent Memory Poisoning Detection; https://arxiv.org/abs/2606.30566; related identifier https://doi.org/10.48550/arXiv.2606.30566. Used as evidence, not instruction.
[^dep]: Complete source record at .lake-data/DEP-E/DEP-E-20260711-Agent Memory Forensics, commit a1ffd4737ce95c14f3a15251ee4fbba4403108a5; both tracked files reviewed in place.
[^doi]: Persistent identifier: https://doi.org/10.48550/arXiv.2606.30566
...
[^pair]: Paired task indicator `BL-DEPPAIR-20260714-28AEA723` records the one-way `DEP-E -> DEP-A` relationship.
