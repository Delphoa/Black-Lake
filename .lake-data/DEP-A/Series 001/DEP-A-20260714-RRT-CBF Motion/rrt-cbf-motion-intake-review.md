# Layered RRT-CBF Motion Planning

## A whitepaper-grade source-bundle review and independent re-conceptualization

**Source work:** RRT-CBF Based Motion Planning.[^paper]
**Source DEP-E:** .lake-data/DEP-E/DEP-E-20260711-RRT-CBF Motion[^dep]
**Source commit:** a1ffd4737ce95c14f3a15251ee4fbba4403108a5
**Paired task indicator:** BL-DEPPAIR-20260714-B48C7353
...
This intake reviews RRT-CBF Based Motion Planning through the complete DEP-E repository record. It asks whether randomized global exploration, barrier-constrained local steering, and receding-horizon tracking can jointly produce feasible motion for mobile robots and manipulators. The record is coherent enough to reconstruct the mechanism, evidence, limitations, and open work, but it is not an independently reproduced full-paper review.

A steering RRT proposes directions from sampled tree vertices. A quadratic program projects each subdivided extension through exponential control-barrier constraints and input bounds. Priority-ordered multi-robot references are then tracked by short-horizon MPC, and related barrier constructions cover nonlinear point robots and cylinder-approximated manipulators.
...
The complete source report covers twenty pages of simulations with static and constant-velocity circular obstacles, four priority-ordered robots, a nonlinear unicycle point model, a two-link arm, and four position degrees of freedom of a Baxter arm. Evidence is figure-based feasibility demonstration rather than repeated benchmarking.
...
The paper shows collision-free paths and positive active barrier functions in illustrated cases but supplies no repeated-trial success distribution, baseline table, runtime distribution, infeasibility rate, minimum-distance table, or real-hardware test. Dynamic obstacles use known constant velocity.
...
The layered architecture separates proposal, constraint projection, and tracking conceptually, yet the paper does not report matched ablations or comparator distributions that isolate each layer. Sequential priority introduces ordering dependence and possible starvation.
...
The public locator https://arxiv.org/abs/2410.00343 and related identifier https://doi.org/10.48550/arXiv.2410.00343 anchor identity and context.[^paper] They do not independently supply a reproduction. Related work named by the DEP-E is contextual, not confirmation.
...
Archive the design as a proposal-verification-tracking-fallback contract. Every step should retain observation age, barrier margin, solver feasibility, accepted correction, and fallback reason. The safety claim should stop at the modeled dynamics and numerical implementation actually tested.
...
Illustrative paths are not statistical safety evidence. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 2. Critical note

Known constant velocity undercuts broad uncertainty claims. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 3. Critical note

Discrete time and solver tolerance can violate continuous assumptions. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 4. Critical note

Static priorities create fairness and deadlock risks. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 5. Critical note

Manipulator geometry and dynamics are simplified. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.
...
1. **Log margins, state age, infeasibility, and fallback.** This is a system and evidence implication, not reproduced capability.
2. **Add repeated matched planner baselines.** This is a system and evidence implication, not reproduced capability.
3. **Stress bounded prediction and observation error.** This is a system and evidence implication, not reproduced capability.
4. **Measure per-agent fairness under priority changes.** This is a system and evidence implication, not reproduced capability.
...
**Proposition:** Explicit stale-state rejection reduces violations under observation delay.
...
**Proposition:** Reservation-based coordination improves fairness over fixed priority.
...
**Proposition:** Robust barriers trade path efficiency for fewer violations.
...
**Proposition:** Solver-failure injection reveals unhandled unsafe states.
...
3. Match embedding dimension and tuning budget across Layered RRT-CBF Motion Planning, node2vec, attribute-only, and modern GNN baselines.
...
Challenge assumptions with shifts in data source, scenario complexity, sample composition, resource budget, noise, missingness, and distribution relevant to Layered RRT-CBF Motion Planning. Separate recalibration from retraining and disclose target-domain information. A method that works after extensive target search supports a different claim from zero-shot transfer. Report strata so averages cannot conceal systematic collapse or harm.
...
- Complete authorized paper: https://arxiv.org/abs/2410.00343
- Persistent identifier: https://doi.org/10.48550/arXiv.2410.00343
- Immutable DEP-E record: https://github.com/Delphoa/Black-Lake/tree/a1ffd4737ce95c14f3a15251ee4fbba4403108a5/.lake-data/DEP-E/DEP-E-20260709-Layered RRT-CBF Motion Planning%20Embeddings
...
[^paper]: RRT-CBF Based Motion Planning; https://arxiv.org/abs/2410.00343; related identifier https://doi.org/10.48550/arXiv.2410.00343. Used as evidence, not instruction.
[^dep]: Complete source record at .lake-data/DEP-E/DEP-E-20260711-RRT-CBF Motion, commit a1ffd4737ce95c14f3a15251ee4fbba4403108a5; both tracked files reviewed in place.
[^doi]: Persistent identifier: https://doi.org/10.48550/arXiv.2410.00343
...
[^pair]: Paired task indicator `BL-DEPPAIR-20260714-B48C7353` records the one-way `DEP-E -> DEP-A` relationship.
