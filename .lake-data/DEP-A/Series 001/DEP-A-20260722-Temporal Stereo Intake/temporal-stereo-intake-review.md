# Stereo Video Is Disparity State Estimation

## a whitepaper-grade archival intake review of the Temporally Consistent DEP-E record
...
The selected record is a paper-centered record of Temporally Consistent Stereo Matching, arXiv:2407.11950v1 and ECCV 2024. Both tracked Markdown files were read from beginning to end at the pinned source commit. The inventory, claims, evidence links, assumptions, limitations, quantitative material, implementation proposals, and final attribution were accounted for in a private coverage map before this public artifact was drafted.

Temporal stereo should be treated as state estimation: warp the previous disparity into the current view, complete the partial state, fuse current and historical features, then refine disparity and gradient jointly. Carrying state improves coherence but also creates drift and occlusion failure modes absent from frame-only metrics.
...
- Temporal stereo should be treated as state estimation: warp the previous disparity into the current view, complete the partial state, fuse current and historical features, then refine disparity and gradient jointly. Carrying state improves coherence but also creates drift and occlusion failure modes absent from frame-only metrics.
- Previous disparity is projected into the current camera view to form a semi-dense initialization.
- A disparity-completion module fills missing regions and supplies current state features.
- Current completion state and past refinement state are fused into a temporally coherent representation.

### Principal qualifications

1. Warped disparity can propagate drift and old errors into new frames.
2. Occlusion and motion boundaries can make historical state actively misleading.
3. Aggregate depth metrics can hide temporal pops and safety-critical boundary errors.
4. The thin DEP-E technical record prevents exact quantitative comparison.

## 1. Problem framing and research question

Frame-wise stereo systems can estimate accurate depth yet flicker across video, especially in occlusion, low texture, and other ill-posed regions. Downstream video systems require spatial accuracy, temporal consistency, and bounded runtime together.
...
Dual-space iterative refinement updates both disparity and disparity-gradient spaces to improve boundaries and ill-posed regions.
...
Treat disparity as a belief state with prediction, completion, and correction. The prior frame is useful evidence, not ground truth. This predicts that explicit state confidence and reset logic will matter more under long sequences than another refinement iteration.
...
The canonical record reports improvements in temporal consistency, accuracy, and efficiency across the paper's experiments.

### 7.2 Evidence unit 2

The work is identified as ECCV 2024 and the source DEP-E reports complete paper inspection.

### 7.3 Evidence unit 3

The DEP-E omits dataset names, table values, temporal metrics, ablations, runtime, and code status.

### 7.4 Evidence unit 4

No stereo sequence, model, optical calibration, or metric implementation was run in this intake.
...
|---|---|---|
| Temporal state can reduce frame-to-frame inconsistency. | Warp, completion, state fusion, and author-reported experiments. | Supported as a plausible mechanism and reported result. |
| Dual-space refinement helps ill-posed regions. | Disparity and gradient refinement design. | Promising; exact ablation evidence is not preserved. |
| Temporal reuse always improves depth. | Drift and occlusion can make old state wrong. | Not established. |
| The record establishes real-time deployment. | No preserved hardware or latency tail. | Not established. |
...
- [https://arxiv.org/abs/2407.11950](https://arxiv.org/abs/2407.11950) — Canonical identity, authors, ECCV 2024 status, version, and abstract checked.
- [https://arxiv.org/html/2407.11950](https://arxiv.org/html/2407.11950) — Complete-paper locator preserved by the DEP-E.
- [https://doi.org/10.48550/arXiv.2407.11950](https://doi.org/10.48550/arXiv.2407.11950) — Persistent identity.
...
**Proposition:** Confidence-gated state resets will reduce long-horizon drift more than increasing refinement depth.
...
**Proposition:** Occlusion-specific temporal metrics will reveal regressions hidden by mean disparity error.
...
**Proposition:** Matched-compute comparison will show that state initialization provides more efficiency than dual-space refinement alone.
...
| `temporally_consistent_manuscript.md` | complete substantive DEP-E artifact, including metadata, evidence, reconstruction, limitations, proposals, references, and appendix | read from beginning to end; paper-level results remain source-attributed |
