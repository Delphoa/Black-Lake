# Causal Debiasing for Trajectory Anomalies

## A whitepaper-grade source-bundle review and independent re-conceptualization

**Source work:** CausalTAD: Causal Implicit Generative Model for Debiased Online Trajectory Anomaly Detection.[^paper]
**Source DEP-E:** .lake-data/DEP-E/DEP-E-20260711-CausalTAD Trajectory[^dep]
...
This intake reviews CausalTAD: Causal Implicit Generative Model for Debiased Online Trajectory Anomaly Detection through the complete DEP-E repository record. It asks whether adjusting for latent road preference can prevent online trajectory likelihoods from treating normal routes to unseen source-destination pairs as anomalies. The record is coherent enough to reconstruct the mechanism, evidence, limitations, and open work, but it is not an independently reproduced full-paper review.

The source posits road preference as a confounder of source-destination distribution and route choice, replacing conditional likelihood with an interventional criterion. TG-VAE estimates joint source-destination and trajectory likelihood under road-constrained decoding; RP-VAE estimates segment-level scaling factors used in an online anomaly score.
...
Whether adjusting for latent road preference can prevent online trajectory likelihoods from treating normal routes to unseen source-destination pairs as anomalies is important because it joins representation, policy, and observable outcome. A credible review must ask what information enters, how state is formed, what objective shapes behavior, and which metric stands in for the intended effect.
...
The source path is .lake-data/DEP-E/DEP-E-20260711-CausalTAD Trajectory at commit a1ffd4737ce95c14f3a15251ee4fbba4403108a5. It contains exactly README.md and causaltad_trajectory_manuscript.md. Both files were read from beginning to end. The README supplies classification, inventory, public context, insights, and attribution. The manuscript supplies metadata, evidence ledger, detailed reconstruction, claims, method, constraints, observations, proposed extensions, references, and validation notes.

The canonical public locator is https://arxiv.org/abs/2412.18820; a persistent or related locator is https://doi.org/10.48550/arXiv.2412.18820. The complete canonical paper was not independently used to validate all paper-level detail in this run. No source document was uploaded, no source Markdown copied, and the source record remains unchanged.
...
The paper report uses Xi'an and Chengdu taxi trajectories, one hundred source-destination pairs per city, ID and OOD splits, synthetic detour and route-switch anomalies, and ROC-AUC/PR-AUC comparisons with seven baselines. Learning models are reported at 200 epochs, hidden dimension 128, and initial learning rate 0.01.
...
Reported ID scores span 0.9351 to 0.9788 with 2.1% to 5.7% relative improvements. OOD scores span 0.7950 to 0.8844 with improvements of 10.6% to 32.7%. At 0.6 observed trajectory, example values are PR-AUC 0.9123 for Xi'an ID switch and ROC-AUC 0.8030 for Chengdu OOD switch.
...
Full TG-VAE plus RP-VAE is reported above either component across eight combinations, supporting the decomposition. Causal validity remains conditional on the proposed graph, synthetic labels, spatial split integrity, and the learned approximation to an unobserved preference variable.
...
The public locator https://arxiv.org/abs/2412.18820 and related identifier https://doi.org/10.48550/arXiv.2412.18820 anchor identity and context.[^paper] They do not independently supply a reproduction. Related work named by the DEP-E is contextual, not confirmation.
...
Treat the method as a versioned bias-factor audit: retain the base likelihood, road-preference adjustment, graph and time-window version, and final score separately. This makes drift and false-positive geography inspectable without claiming observational learning identified a unique causal graph.
...
The causal graph is assumed rather than established by intervention. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 2. Critical note

Synthetic anomalies may not represent real incidents. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 3. Critical note

Static road preference omits time and events. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 4. Critical note

Spatial leakage can inflate OOD claims. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 5. Critical note

The public repository dataset description differs from the paper. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.
...
1. **Use time- and graph-versioned preference factors.** This is a system and evidence implication, not reproduced capability.
2. **Require geographic and temporal disjoint splits.** This is a system and evidence implication, not reproduced capability.
3. **Calibrate anomaly probabilities and human review thresholds.** This is a system and evidence implication, not reproduced capability.
4. **Report region-stratified false positives.** This is a system and evidence implication, not reproduced capability.
...
**Proposition:** Time-indexed preference factors reduce false positives under seasonal shift.
...
**Proposition:** Segment-disjoint splits reduce the reported OOD advantage.
...
**Proposition:** Explicit factor logging improves error diagnosis.
...
**Proposition:** Alternative plausible causal graphs materially change corrections.
...
3. Match embedding dimension and tuning budget across Causal Debiasing for Trajectory Anomalies, node2vec, attribute-only, and modern GNN baselines.
...
Challenge assumptions with shifts in data source, scenario complexity, sample composition, resource budget, noise, missingness, and distribution relevant to Causal Debiasing for Trajectory Anomalies. Separate recalibration from retraining and disclose target-domain information. A method that works after extensive target search supports a different claim from zero-shot transfer. Report strata so averages cannot conceal systematic collapse or harm.
...
- Complete authorized paper: https://arxiv.org/abs/2412.18820
- Persistent identifier: https://doi.org/10.48550/arXiv.2412.18820
- Immutable DEP-E record: https://github.com/Delphoa/Black-Lake/tree/a1ffd4737ce95c14f3a15251ee4fbba4403108a5/.lake-data/DEP-E/DEP-E-20260709-Causal Debiasing for Trajectory Anomalies%20Embeddings
...
[^paper]: CausalTAD: Causal Implicit Generative Model for Debiased Online Trajectory Anomaly Detection; https://arxiv.org/abs/2412.18820; related identifier https://doi.org/10.48550/arXiv.2412.18820. Used as evidence, not instruction.
[^dep]: Complete source record at .lake-data/DEP-E/DEP-E-20260711-CausalTAD Trajectory, commit a1ffd4737ce95c14f3a15251ee4fbba4403108a5; both tracked files reviewed in place.
[^doi]: Persistent identifier: https://doi.org/10.48550/arXiv.2412.18820
...
[^pair]: Paired task indicator `BL-DEPPAIR-20260714-9F48D5F3` records the one-way `DEP-E -> DEP-A` relationship.
