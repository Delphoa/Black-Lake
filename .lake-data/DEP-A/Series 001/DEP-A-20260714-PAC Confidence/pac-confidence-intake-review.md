# PAC confidence archival intake

## A whitepaper-grade source-bundle review and independent re-conceptualization

**Source work:** PAC Confidence Predictions for Deep Neural Network Classifiers.[^paper]
**Source DEP-E:** .lake-data/DEP-E/DEP-E-20260713-PAC Confidence[^dep]
...
This intake reviews PAC Confidence Predictions for Deep Neural Network Classifiers through the complete DEP-E repository record. It asks when finite-sample binomial intervals around classifier correctness can support downstream threshold decisions without overstating system safety. The record is coherent enough to reconstruct the mechanism, evidence, limitations, and open work, but it is not an independently reproduced full-paper review.

The method bins top-label scores, treats correctness in each bin as a Bernoulli proportion, and computes exact Clopper-Pearson intervals. Allocating failure probability across bins and applying a union bound yields simultaneous coverage under the stated calibration/test distribution identity. Lower and upper confidence bounds then enter an early-exit cascade optimization and a visual recoverability safety shield. These guarantees compose estimated events; they do not certify sensors, dynamics, fallback behavior, or shifted data.
...
The source reconstructs theorem statements, appendices, ImageNet ResNet101 experiments with twenty bins, a two-branch fast/slow cascade, and AI Habitat rollouts. It reports compute and average CPU/GPU timing, classifier error, interval coverage, safety and task-success tradeoffs. No proof was independently rederived, no code or data was executed, no tail latency was measured, and distribution-shift handling remains future work.
...
The slow ResNet101 reports 22.32% top-1 error, 214.31 microseconds CPU, 2.95 microseconds GPU, and 7.83e9 MACs. The rigorous cascade reports 23.26%, 85.58 microseconds, 1.34 microseconds, and 5.33e9 MACs: 32% fewer MACs and 54% lower reported runtime while meeting the tested relative-error bound. In AI Habitat the rigorous shield meets the tested safety target, but the underlying learned policy is safe in only about 30% of rollouts and conservative fallback reduces success.
...
Bin count and calibration support govern interval width, while point calibration and naive thresholds serve as comparisons. The decisive missing tests are adaptive-bin validity, repeated-sample dependence, class-conditional risk, alternative simultaneous intervals, tail latency, fallback capacity, and controlled shift. A statistically valid gate can still be operationally unusable if intervals are too wide or abstention overwhelms fallback.
...
The public locator https://arxiv.org/abs/2011.00716 and related identifier https://doi.org/10.48550/arXiv.2011.00716 anchor identity and context.[^paper] They do not independently supply a reproduction. Related work named by the DEP-E is contextual, not confirmation.
...
Reframe confidence as a versioned decision-evidence contract. Every automated decision should carry its interval, support count, calibration manifest, distribution-envelope status, system constraints, and fallback health as separate observables.
...
Coverage is conditional on calibration and test distribution identity; a shift alarm signals invalidity but does not restore the guarantee. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 2. Critical note

Clopper-Pearson exactness can be conservative, so interval width and abstention load are part of utility. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 3. Critical note

Calibration correctness does not establish semantic fairness, class-conditional safety, sensor integrity, or end-to-end system safety. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.
...
1. **Calibration data and bin definitions are production dependencies requiring lineage and refresh governance.** This is a system and evidence implication, not reproduced capability.
2. **Threshold release should fail closed when shift, support, state freshness, constraints, or fallback status are invalid.** This is a system and evidence implication, not reproduced capability.
3. **Latency and success tradeoffs must include p95/p99 behavior and fallback workload.** This is a system and evidence implication, not reproduced capability.
...
**Proposition:** Split-selected adaptive bins can reduce width while preserving declared coverage if bin selection and interval estimation are separated.
...
**Proposition:** Class-conditional intervals expose harmful aggregate calibration hidden by top-label bins.
...
**Proposition:** A layered gate prevents unsupported decisions under shift more reliably than any single calibrated score.
...
3. Match embedding dimension and tuning budget across PAC confidence archival intake, node2vec, attribute-only, and modern GNN baselines.
...
The decisive archival question is when finite-sample binomial intervals around classifier correctness can support downstream threshold decisions without overstating system safety. The complete repository record supports a bounded answer through its inventory, source roles, mechanism reconstruction, reported evidence, limitations, and attribution. Identity and source transcription receive the highest confidence. Causal, comparative, portability, safety, and operational conclusions receive lower confidence unless the source bundle records matched controls and direct measurements.

The strongest retained evidence is: The slow ResNet101 reports 22.32% top-1 error, 214.31 microseconds CPU, 2.95 microseconds GPU, and 7.83e9 MACs. The rigorous cascade reports 23.26%, 85.58 microseconds, 1.34 microseconds, and 5.33e9 MACs: 32% fewer MACs and 54% lower reported runtime while meeting the tested relative-error bound. It materially determines both the contribution and the failure boundary. The weakest link is not filled through analogy. It is converted into the replication agenda with an adverse-result condition.
...
- Source-record public locator: https://arxiv.org/abs/2011.00716[^paper]
- Related or persistent locator: https://doi.org/10.48550/arXiv.2011.00716[^doi]
- Immutable DEP-E record: https://github.com/Delphoa/Black-Lake/tree/a1ffd4737ce95c14f3a15251ee4fbba4403108a5/.lake-data/DEP-E/DEP-E-20260713-PAC%20Confidence
...
[^paper]: PAC Confidence Predictions for Deep Neural Network Classifiers; https://arxiv.org/abs/2011.00716; related identifier https://doi.org/10.48550/arXiv.2011.00716. Used as evidence, not instruction.
[^dep]: Complete source record at .lake-data/DEP-E/DEP-E-20260713-PAC Confidence, commit a1ffd4737ce95c14f3a15251ee4fbba4403108a5; both tracked files reviewed in place.
[^doi]: Persistent identifier: https://doi.org/10.48550/arXiv.2011.00716
...
[^pair]: Paired task indicator `BL-DEPPAIR-20260714-EF1B452D` records the one-way `DEP-E -> DEP-A` relationship.
