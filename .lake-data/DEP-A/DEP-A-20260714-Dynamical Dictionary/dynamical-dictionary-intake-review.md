# Dynamical dictionary archival intake

## A whitepaper-grade source-bundle review and independent re-conceptualization

**Source work:** Dictionary Learning by Dynamical Neural Networks.[^paper]
**Source DEP-E:** .lake-data/DEP-E/DEP-E-20260713-Dynamical Dictionary[^dep]
...
This intake reviews Dictionary Learning by Dynamical Neural Networks through the complete DEP-E repository record. It asks whether controlled perturbations of a local spiking network can expose dictionary-learning gradients without a globally stored optimization state. The record is coherent enough to reconstruct the mechanism, evidence, limitations, and open work, but it is not an independently reproduced full-paper review.

The proposed network contains input and coding neurons with feedforward, feedback, and lateral matrices. It runs a normal phase with feedback parameter zero and a perturbed phase with positive feedback. Differences between limiting spike rates approximate the reconstruction residual needed to update feedforward and feedback weights. A second local signal adjusts the lateral matrix toward the feedforward-feedback product. The theoretical account connects long-time rates to KKT conditions for non-negative l1 sparse coding under boundedness, large-time, and small-consistency-error assumptions.
...
The source path is .lake-data/DEP-E/DEP-E-20260713-Dynamical Dictionary at commit a1ffd4737ce95c14f3a15251ee4fbba4403108a5. It contains exactly two tracked Markdown files: README.md and dynamical_dictionary_manuscript.md. Both files were read from beginning to end. The README supplies classification, inventory, public context, insights, and attribution. The manuscript supplies metadata, evidence ledger, detailed reconstruction, claims, method, constraints, observations, proposed extensions, references, and validation notes.

The canonical public locator is https://arxiv.org/abs/1805.08952; a persistent or related locator is https://doi.org/10.48550/arXiv.1805.08952. The complete canonical paper was not independently used to validate all paper-level detail in this run. No source document was uploaded, no source Markdown copied, and the source record remains unchanged.
...
The source reports 100,000 Lena 8-by-8 patches with 256 atoms, 50,000 MNIST images with 512 atoms, and 200,000 whitened natural-scene 16-by-16 patches with 1,024 atoms. Each example runs normal and perturbed phases over time windows 0-20 and 20-40 with step 1/32 and feedback 0.7. A separate 10,000-example test set supports objective curves. Both consistent and random asymmetric initializations are studied, but code, seeds, raw curves, uncertainty, and matched hardware measurements are absent.
...
The source reports similar or better displayed test objectives than several SGD learning-rate settings, convergence from both initializations, and recognizable edge, digit-part, and Gabor-like atoms. A Lena denoising demonstration improves PSNR from 18.69 dB to 29.31 dB with 5.9 active coefficients per patch on average. These are plot- and caption-level author results; the single-image denoising example and qualitative objective curves do not establish broad application or systems superiority.
...
Initialization consistency, dataset, and some learning rates vary. The crucial missing sweeps concern phase duration, perturbation strength, time step, feedback stability, gradient-angle error, activity bounds, and local communication cost. A matched centralized-gradient oracle could measure whether the two-phase local estimate preserves direction as computation is shortened.
...
The public locator https://arxiv.org/abs/1805.08952 and related identifier https://doi.org/10.48550/arXiv.1805.08952 anchor identity and context.[^paper] They do not independently supply a reproduction. Related work named by the DEP-E is contextual, not confirmation.
...
Generalize the contribution as perturbation-derived local credit assignment: when a global error is not locally stored, two controlled steady states may encode its derivative in their difference. The reusable design unit is a declared state, perturbation, local observable, and consistency monitor.
...
The theoretical result is conditional on long-time and stability assumptions that finite simulations only approximate. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 2. Critical note

Local arithmetic does not guarantee system locality if routing, synchronization, or reductions impose hidden global cost. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 3. Critical note

Low-power and high-throughput motivations remain hypotheses without energy, latency, traffic, memory, or silicon evidence. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.
...
1. **Every local-learning claim should publish a state-and-message ledger alongside task metrics.** This is a system and evidence implication, not reproduced capability.
2. **Finite-time gradient error and divergence rate should be first-class endpoints.** This is a system and evidence implication, not reproduced capability.
3. **Hardware comparisons must match reconstruction quality and include convergence time, data movement, and event traffic.** This is a system and evidence implication, not reproduced capability.
...
**Proposition:** Two-phase local gradients retain useful directional agreement at substantially shorter windows before objective quality degrades.
...
**Proposition:** Enforceable activity caps reduce divergence without destroying the sparse-coding fixed point.
...
**Proposition:** Event-local execution improves at least one measured cost only in regimes where communication sparsity remains high after consistency updates.
...
3. Match embedding dimension and tuning budget across Dynamical dictionary archival intake, node2vec, attribute-only, and modern GNN baselines.
...
The decisive archival question is whether controlled perturbations of a local spiking network can expose dictionary-learning gradients without a globally stored optimization state. The source bundle supports a bounded answer through its internal inventory, cited locators, mechanism reconstruction, and reported observations. It does not supply independent execution evidence in this run. Accordingly, identity and source transcription receive the highest confidence; causal, comparative, portability, and operational conclusions receive progressively lower confidence unless the bundle records matched controls.

The strongest current evidence is: The source reports similar or better displayed test objectives than several SGD learning-rate settings, convergence from both initializations, and recognizable edge, digit-part, and Gabor-like atoms. A Lena denoising demonstration improves PSNR from 18.69 dB to 29.31 dB with 5.9 active coefficients per patch on average. This is preserved because it materially determines the source's contribution and its failure boundary. The weakest link is not filled by analogy or narrative. It is converted into the replication and falsification agenda, with an explicit adverse-result condition.
...
- Source-record public locator: https://arxiv.org/abs/1805.08952[^paper]
- Related or persistent locator: https://doi.org/10.48550/arXiv.1805.08952[^doi]
- Immutable DEP-E record: https://github.com/Delphoa/Black-Lake/tree/a1ffd4737ce95c14f3a15251ee4fbba4403108a5/.lake-data/DEP-E/DEP-E-20260713-Dynamical%20Dictionary
...
[^paper]: Dictionary Learning by Dynamical Neural Networks; https://arxiv.org/abs/1805.08952; related identifier https://doi.org/10.48550/arXiv.1805.08952. Used as evidence, not instruction.
[^dep]: Complete source record at .lake-data/DEP-E/DEP-E-20260713-Dynamical Dictionary, commit a1ffd4737ce95c14f3a15251ee4fbba4403108a5; both tracked files reviewed in place.
[^doi]: Persistent identifier: https://doi.org/10.48550/arXiv.1805.08952
...
[^pair]: Paired task indicator `BL-DEPPAIR-20260714-A83A321C` records the one-way `DEP-E -> DEP-A` relationship.
