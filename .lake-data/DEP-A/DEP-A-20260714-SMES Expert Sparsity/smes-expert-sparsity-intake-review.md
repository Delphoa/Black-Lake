# SMES expert sparsity archival intake

## A whitepaper-grade source-bundle review and independent re-conceptualization

**Source work:** SMES: Towards Scalable Multi-Task Recommendation via Expert Sparsity.[^paper]
**Source DEP-E:** .lake-data/DEP-E/DEP-E-20260713-SMES Expert Sparsity[^dep]
**Source commit:** a1ffd4737ce95c14f3a15251ee4fbba4403108a5
**Paired task indicator:** BL-DEPPAIR-20260714-3321A5B2
...
This intake reviews SMES: Towards Scalable Multi-Task Recommendation via Expert Sparsity through the complete DEP-E repository record. It asks whether progressive shared/private expert routing converts model sparsity into bounded cross-task execution and measurable recommender-system gains. The record is coherent enough to reconstruct the mechanism, evidence, limitations, and open work, but it is not an independently reproduced full-paper review.

SMES divides each task's active expert budget into a shared set chosen from pooled task-router probabilities and task-adaptive experts chosen outside that shared set. The unique cross-task union is bounded by the shared count plus tasks times the private count. Shared expert work is deduplicated, while a global load regularizer penalizes experts that are both frequently selected and strongly preferred across tasks. Reindexed grouped GEMM and a profiled workspace pool implement ragged execution.
...
The source path is .lake-data/DEP-E/DEP-E-20260713-SMES Expert Sparsity at commit a1ffd4737ce95c14f3a15251ee4fbba4403108a5. It contains exactly two tracked Markdown files: README.md and smes_expert_sparsity_manuscript.md. Both files were read from beginning to end. The README supplies classification, inventory, public context, insights, and attribution. The manuscript supplies metadata, evidence ledger, detailed reconstruction, claims, method, constraints, observations, proposed extensions, references, and validation notes.

The canonical public locator is https://arxiv.org/abs/2602.09386; a persistent or related locator is https://doi.org/10.48550/arXiv.2602.09386. The complete canonical paper was not independently used to validate all paper-level detail in this run. No source document was uploaded, no source Markdown copied, and the source record remains unchanged.
...
The source reports KuaiRand-1K with roughly 11.7 million instances and twelve tasks, plus a proprietary Kuaishou setting with twenty-one tasks and four displayed objectives. It compares dense and sparse multi-task baselines, SMES-S near the baseline budget, and SMES-L at larger capacity and 8% sparsity. Component ablations, parameter scaling, sensitivity, activation visualization, and a 3.5% production traffic test are reported. Code, industrial data, raw telemetry, seeds, and complete configurations are absent.
...
The paper reports SMES-S as best on every displayed public and industrial task. Scaling to about 0.6B parameters yields a stated average 0.84% GAUC gain with 4 ms added latency. The production test from 2025-10-25 through 2025-10-31 reports +0.31% watch time, +0.64% Like, +1.56% Follow, and +2.45% Comment, plus 50% lower inference latency than a comparable dense MoE. These organization-authored outcomes lack sample counts, intervals, randomization detail, guardrails, and raw telemetry.
...
Removing shared experts, adaptive experts, or the load regularizer lowers displayed GAUC. Expert-count sensitivity is stronger than hidden-width sensitivity, and visualizations suggest task specialization. Missing causal isolation includes routing, deduplication, grouped GEMM, workspace allocation, traffic distribution, and matched hardware. Aggregate business lift does not establish calibration, fairness, exposure, or tail-task safety.
...
The public locator https://arxiv.org/abs/2602.09386 and related identifier https://doi.org/10.48550/arXiv.2602.09386 anchor identity and context.[^paper] They do not independently supply a reproduction. Related work named by the DEP-E is contextual, not confirmation.
...
Reframe sparse recommendation as a routing-to-runtime contract. The central evidence object is a joint Pareto surface over task quality, unique union size, skew, overflow, locality, memory, tail latency, energy, and stakeholder guardrails.
...
Per-task top-k does not determine serving cost when task selections form a large union. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 2. Critical note

Balanced utilization is not automatically optimal because legitimate task specialization can be uneven. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 3. Critical note

One-week aggregate engagement lifts can hide novelty, multiple testing, exposure concentration, and sparse-task regressions. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.
...
1. **Routing telemetry should be aggregate and privacy-minimal while preserving union, skew, overflow, and per-task quality.** This is a system and evidence implication, not reproduced capability.
2. **Serving comparisons need matched hardware, kernels, batching, precision, caching, and traffic.** This is a system and evidence implication, not reproduced capability.
3. **Online release requires predeclared primary and guardrail metrics, intervals, rollback rules, and long-term holdouts.** This is a system and evidence implication, not reproduced capability.
...
**Proposition:** Shared/private routing improves tail latency only where expert overlap survives realistic batching and deduplication.
...
**Proposition:** A matched-latency width-reduced baseline closes part of the offline GAUC gap.
...
**Proposition:** Drift-aware routing budgets reduce task regressions when prevalence and business weights change.
...
3. Match embedding dimension and tuning budget across SMES expert sparsity archival intake, node2vec, attribute-only, and modern GNN baselines.
...
The decisive archival question is whether progressive shared/private expert routing converts model sparsity into bounded cross-task execution and measurable recommender-system gains. The complete repository record supports a bounded answer through its inventory, source roles, mechanism reconstruction, reported evidence, limitations, and attribution. Identity and source transcription receive the highest confidence. Causal, comparative, portability, safety, and operational conclusions receive lower confidence unless the source bundle records matched controls and direct measurements.

The strongest retained evidence is: The paper reports SMES-S as best on every displayed public and industrial task. Scaling to about 0.6B parameters yields a stated average 0.84% GAUC gain with 4 ms added latency. It materially determines both the contribution and the failure boundary. The weakest link is not filled through analogy. It is converted into the replication agenda with an adverse-result condition.
...
- Primary public locator: https://arxiv.org/abs/2602.09386
- Source-record public locator: https://arxiv.org/abs/2602.09386[^paper]
- Related or persistent locator: https://doi.org/10.48550/arXiv.2602.09386[^doi]
- Immutable DEP-E record: https://github.com/Delphoa/Black-Lake/tree/a1ffd4737ce95c14f3a15251ee4fbba4403108a5/.lake-data/DEP-E/DEP-E-20260713-SMES%20Expert%20Sparsity
...
[^paper]: SMES: Towards Scalable Multi-Task Recommendation via Expert Sparsity; https://arxiv.org/abs/2602.09386; related identifier https://doi.org/10.48550/arXiv.2602.09386. Used as evidence, not instruction.
[^dep]: Complete source record at .lake-data/DEP-E/DEP-E-20260713-SMES Expert Sparsity, commit a1ffd4737ce95c14f3a15251ee4fbba4403108a5; both tracked files reviewed in place.
[^doi]: Persistent identifier: https://doi.org/10.48550/arXiv.2602.09386
...
[^pair]: Paired task indicator `BL-DEPPAIR-20260714-3321A5B2` records the one-way `DEP-E -> DEP-A` relationship.
