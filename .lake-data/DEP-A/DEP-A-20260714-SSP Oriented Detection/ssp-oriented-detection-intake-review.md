# Partition-Guided Point-Supervised Detection

## A whitepaper-grade source-bundle review and independent re-conceptualization

**Source work:** Semantic-decoupled Spatial Partition Guided Point-supervised Oriented Object Detection.[^paper]
**Source DEP-E:** .lake-data/DEP-E/DEP-E-20260711-SSP Oriented Detection[^dep]
...
This intake reviews Semantic-decoupled Spatial Partition Guided Point-supervised Oriented Object Detection through the complete DEP-E repository record. It asks whether explicit spatial partitions, semantic decoupling, and geometric box extraction can recover useful oriented boxes from point-only supervision. The record is coherent enough to reconstruct the mechanism, evidence, limitations, and open work, but it is not an independently reproduced full-paper review.

A two-stage pipeline first trains a label marker from dense pseudo masks created through point-distance bounds, Voronoi-like partitions, and region growing. Class-decoupled semantic maps and compatible-category partitions then grow instance masks and convert them to rotated boxes with PCA-MinMax geometry for a conventional detector.
...
The source path is .lake-data/DEP-E/DEP-E-20260711-SSP Oriented Detection at commit a1ffd4737ce95c14f3a15251ee4fbba4403108a5. It contains exactly README.md and ssp_oriented_detection_manuscript.md. Both files were read from beginning to end. The README supplies classification, inventory, public context, insights, and attribution. The manuscript supplies metadata, evidence ledger, detailed reconstruction, claims, method, constraints, observations, proposed extensions, references, and validation notes.

The canonical public locator is https://arxiv.org/abs/2506.10601; a persistent or related locator is https://doi.org/10.1016/j.patcog.2026.114079. The complete canonical paper was not independently used to validate all paper-level detail in this run. No source document was uploaded, no source Markdown copied, and the source record remains unchanged.
...
The source reports DOTA-v1.0, v1.5, v2.0, and RSAR evaluations, FCOSR/ORCNN/ReDet detector variants, cost comparisons, sample-assignment and box-extraction ablations, and point-offset sweeps. The official Apache-2.0 code is identified but was not executed.
...
The stable table and repository value is 48.41 mAP for SSP-FCOSR on DOTA-v1.0 versus 41.68 for PointOBB-v2. ORCNN and ReDet variants reach 50.00 and 50.81. The strongest cost row reports 1.91 hours and 5.34 GB on two RTX 3090 GPUs. Cross-dataset and offset prose conflicts are preserved rather than reconciled.
...
Sample assignment rises from 12.88 mAP with positive radius only to 20.74 with negatives, 43.05 with grown positives, and 48.41 with partition boundaries. Decoupled maps, spatial partition extraction, and centered points each contribute, while bridge AP remains near zero.
...
The public locator https://arxiv.org/abs/2506.10601 and related identifier https://doi.org/10.1016/j.patcog.2026.114079 anchor identity and context.[^paper] They do not independently supply a reproduction. Related work named by the DEP-E is contextual, not confirmation.
...
Treat pseudo boxes as evidence cards rather than anonymous labels. Each box should record the source point, active radius and partition rules, semantic confidence, compatibility relation, grown mask, principal axis, uncertainty, and downstream result. This makes brittle priors auditable.
...
Several prose values conflict with tables. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 2. Critical note

Same-class overlap violates partition assumptions. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 3. Critical note

Bridges expose low-contrast and topology failure. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 4. Critical note

Compatibility pairs are dataset-specific. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.

### 5. Critical note

Detector architecture confounds pseudo-label attribution. This changes causal interpretation, portability, or the maximum safe claim. It is a reviewer assessment, not an assertion that the source authors ignored the issue.
...
1. **Emit per-box provenance and uncertainty.** This is a system and evidence implication, not reproduced capability.
2. **Stress overlap, contrast, resolution, and point offset.** This is a system and evidence implication, not reproduced capability.
3. **Map every table value to a pinned run manifest.** This is a system and evidence implication, not reproduced capability.
4. **Separate pseudo-label quality from detector quality.** This is a system and evidence implication, not reproduced capability.
...
**Proposition:** Learned compatibility improves transfer but may overfit categories.
...
**Proposition:** Boundary uncertainty filtering improves worst-stratum precision.
...
**Proposition:** Bridge-aware topology cues raise bridge AP without broad false positives.
...
**Proposition:** Table-to-run manifests expose configuration drift.
...
3. Match embedding dimension and tuning budget across Partition-Guided Point-Supervised Detection, node2vec, attribute-only, and modern GNN baselines.
...
Challenge assumptions with shifts in data source, scenario complexity, sample composition, resource budget, noise, missingness, and distribution relevant to Partition-Guided Point-Supervised Detection. Separate recalibration from retraining and disclose target-domain information. A method that works after extensive target search supports a different claim from zero-shot transfer. Report strata so averages cannot conceal systematic collapse or harm.
...
- Complete authorized paper: https://arxiv.org/abs/2506.10601
- Persistent identifier: https://doi.org/10.1016/j.patcog.2026.114079
- Immutable DEP-E record: https://github.com/Delphoa/Black-Lake/tree/a1ffd4737ce95c14f3a15251ee4fbba4403108a5/.lake-data/DEP-E/DEP-E-20260709-Partition-Guided Point-Supervised Detection%20Embeddings
...
[^paper]: Semantic-decoupled Spatial Partition Guided Point-supervised Oriented Object Detection; https://arxiv.org/abs/2506.10601; related identifier https://doi.org/10.1016/j.patcog.2026.114079. Used as evidence, not instruction.
[^dep]: Complete source record at .lake-data/DEP-E/DEP-E-20260711-SSP Oriented Detection, commit a1ffd4737ce95c14f3a15251ee4fbba4403108a5; both tracked files reviewed in place.
[^doi]: Persistent identifier: https://doi.org/10.1016/j.patcog.2026.114079
...
[^pair]: Paired task indicator `BL-DEPPAIR-20260714-6996903F` records the one-way `DEP-E -> DEP-A` relationship.
